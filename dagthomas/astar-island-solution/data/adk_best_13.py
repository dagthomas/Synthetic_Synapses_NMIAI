# ADK Research Agent Best Experiment #13
# Score: avg=87.424, improvement=+0.018
# Timestamp: 2026-03-20T18:17:59.074103+00:00
...
def experimental_pred_fn(state: dict, global_mult, fk_buckets) -> np.ndarray:
    ...
    # 2. Apply Density-Aware Adjustment to Prior BEFORE empirical blending
    ...
    if global_mult.observed.sum() > 0:
        ...
        # Structural fix: Drop the artificial lower bound on volatile classes
        # This allows deep collapse rounds (R3) to properly push settlement prob down
        # The +smooth ensures it's mathematically bounded by the observation volume
        ratio[0] = np.clip(ratio[0], 0.6, 1.4)      # Empty can swing more
        ratio[5] = np.clip(ratio[5], 0.85, 1.15)    # Mountain
        for c in (1, 2, 3):
            ratio[c] = np.clip(ratio[c], 0.02, 2.5) # Volatile classes can crash to 2% (instead of 15%)
        ratio[4] = np.clip(ratio[4], 0.5, 1.8)      # Forest
