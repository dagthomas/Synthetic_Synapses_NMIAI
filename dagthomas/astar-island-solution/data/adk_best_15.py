# ADK Research Agent Best Experiment #15
# Score: avg=91.320, improvement=+0.218
# Timestamp: 2026-03-20T10:26:01.216374+00:00
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers, fk_buckets: FeatureKeyBuckets) -> np.ndarray:
    ...
    for i, fk in enumerate(unique_keys):
        ...
        if b_count > 0:
            bw = min(0.5, b_count / 100.0)
            vector += (base_emp[b_key]["sums"] / b_count) * bw
            total_w += bw
            
        if total_w > 0:
            fk_empiricals[i] = vector / total_w
            # Equivalent total observation count for strength calculation
            # Use max to avoid double counting the same observations!
