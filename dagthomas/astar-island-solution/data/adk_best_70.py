# ADK Research Agent Best Experiment #70
# Score: avg=91.271, improvement=+0.028
# Timestamp: 2026-03-20T13:11:23.486161+00:00
...
def experimental_pred_fn(state: dict, global_mult, fk_buckets) -> np.ndarray:
    ...
    if global_mult.observed.sum() > 0:
        ...
        # Port Boost on coastal cells (only if not total collapse)
        ...
        if sett_survival_ratio > 0.5:
            ...
            # Scale modifier by regime: stronger effect if ratio is very high
                ...
                for sx in range(w):
                    if (sx, sy) in sett_locs:
                        if density_map[sy, sx] >= 2:
                            pred[sy, sx, 1] *= (1.0 - mod_strength) # Penalty for dense
                        elif density_map[sy, sx] <= 1:
                            pred[sy, sx, 1] *= (1.0 + mod_strength) # Bonus for sparse
