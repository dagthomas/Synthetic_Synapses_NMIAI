# ADK Research Agent Best Experiment #8
# Score: avg=87.273, improvement=+0.011
# Timestamp: 2026-03-20T18:23:43.422911+00:00
...
def blur_channel(channel: np.ndarray, center_weight=0.8) -> np.ndarray:
    """Simple 3x3 blur using shift operations."""
    side_weight = (1.0 - center_weight) / 8.0
    
    # Pad channel to handle edges
...
def experimental_pred_fn(state: dict, global_mult, fk_buckets) -> np.ndarray:
    ...
    # ==== SPATIAL SMOOTHING FOR SETTLEMENTS AND PORTS ====
    # Spread a tiny bit of probability to neighbors to simulate expansion/contagion
    pred[:, :, 1] = blur_channel(pred[:, :, 1], center_weight=0.9)
    pred[:, :, 2] = blur_channel(pred[:, :, 2], center_weight=0.9)
