# FAILED: Crashed on round2 seed 0: Cannot cast ufunc 'divide' output from dtype('O') to dtype('float64') with casting rule 'same_kind'
# Direction: Increase the `settlement` class `clamp_range` upper bound from 2.5 to 2.75. This allows the Bayesian
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    try:
        ...
        p = {"prior_w": 2.0, "emp_max": 3.0, "multi_alpha": 0.3}
    ...
    for c in range(NUM_CLASSES):
        low, high = clamp_ranges.get(c, (0.5, 2.0))
        ratio[c] = np.clip(ratio[c], low, high)

    pred *= ratio
    pred /= np.maximum(pred.sum(axis=-1, keepdims=True), 1e-10)

    if multi_store is not None and len(multi_store) > 0:
        avg_pred = np.mean(multi_store, axis=0)
        alpha = p.get("multi_alpha", 0.3)
        pred = pred * (1.0 - alpha) + avg_pred * alpha
