# Score: avg=83.45
# Direction: Adjust the `settlement` class clamp range from [0.15, 2.5] to [0.12, 2.8] to better accommodate high
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    if obs_expansion_radius is not None:
        sett_locs = np.argwhere(np.array(settlements) > 0)
        if len(sett_locs) > 0:
            dist = distance_transform_cdt(np.array(settlements) == 0, metric='taxicab')
