# Score: avg=84.17
# Direction: Since settlement dominates error (4.5% of KL) and the baseline (91.8) beats all recent attempts, red
# Error: None
...
def experimental_pred_fn(state, global_mult, fk_buckets):
    ...
    for i in range(N):
        ...
        mults[2] = np.clip(mults[2], 0.12, 2.0)
        mults[3] = np.clip(mults[3], 0.12, 2.0)
    ...
    port_mult = port_prior_boost * (1.0 + 1.5 * np.exp(-0.5 * (dist_to_coast / 2.0)**2))
