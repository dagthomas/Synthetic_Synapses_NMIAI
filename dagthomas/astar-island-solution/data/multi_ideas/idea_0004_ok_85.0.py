# Score: avg=84.98
# Direction: Since settlement dominates error (4.5% KL), try increasing **settlement_coarse_max calibration weigh
# Error: None
...
def experimental_pred_fn(state, global_mult, fk_buckets):
    ...
    # Coastal/inland power split: coastal settlements die 2x more
    ...
    empty_near = 0.70 + 0.20 * regime
    empty_far = 1.05 + 0.05 * regime
    empty_mult = empty_near + (empty_far - empty_near) * sigmoid_rise
    empty_mult[~land_mask] = 1.0

    # Final Adjustment
    final_probs = probs.copy()
    final_probs[..., 0] *= empty_mult
    final_probs[..., 1] *= sett_mult
