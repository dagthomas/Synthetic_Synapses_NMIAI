# Score: avg=86.97
# Direction: Increase the settlement class upper clamp from 2.5 to 2.7 to account for the dominant KL divergence 
# Error: None
...
def experimental_pred_fn(state: dict, global_mult,
    ...
    mult[1] = np.clip(mult[1], 0.15, 2.7)
    mult[2] = np.clip(mult[2], 0.15, 2.7)
    ...
    if is_sett.any():
        ...
        if obs_expansion_radius is not None:
            mask_out = dist_map > obs_expansion_radius
            pred[mask_out, 1] *= 0.01
            pred[mask_out, 2] *= 0.01
