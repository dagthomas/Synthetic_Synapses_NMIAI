...
# Direction: Increase the `coarse_max` calibration weight for the `settlement` class from 2.0 to 2.15 to sharpen 
...
def experimental_pred_fn(state: dict, global_mult,
    ...
    if growth_front_map is not None:
        pred[:, :, 1] *= np.maximum(1.0, growth_front_map)
    ...
    for r in range(MAP_H):
        for c in range(MAP_W):
            ...
            pred[r, c] = np.maximum(pred[r, c], 0.005)
