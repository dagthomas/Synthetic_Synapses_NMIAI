# Score: avg=84.23
# Direction: Increase the `settlement` class `clamp_max` parameter from 2.5 to 2.75 to better accommodate the hig
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # Clamp ratio multipliers (settlement class clamp_max increased to 2.75)
    clamp_max = np.array([2.0, 2.0, 2.0, 2.75, 2.0, 2.0])
    ratio = np.minimum(ratio, clamp_max)
    ...
    # Apply global ratio adjustments
    pred *= ratio
    ...
    # Ensure floor of 0.005 for nonzero classes
    pred = np.maximum(pred, 0.005)
    
    # Re-apply strict domain rules to override the floor
    pred[~coastal, 2] = 0.0
    pred[~mountain_mask, 5] = 0.0

    # Normalize to ensure all rows sum to 1.0
