# FAILED: Crashed on round2 seed 0: 'int' object has no attribute 'upper'
# Direction: Increase the `settlement` class calibration weight `coarse_max` from 2.0 to 2.15 to better calibrate
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    cal_params = {
        ...
        'cal_coarse_base': 0.5, 'cal_coarse_divisor': 100.0, 'cal_coarse_max': 2.15,
    ...
    ratio = package_mult_ratio = obs / exp_arr
    ...
    mountain_val = 1
    if isinstance(TERRAIN_TO_CLASS, dict):
        for k, v in TERRAIN_TO_CLASS.items():
            if k.upper() == 'MOUNTAIN':
                mountain_val = v
                break
    probs[grid != mountain_val, 5] = 0.0

    probs[probs > 0] = np.maximum(probs[probs > 0], 0.005)
    
    row_sums = probs.sum(axis=-1, keepdims=True)
    probs = np.where(row_sums > 0, probs / row_sums, 1.0 / NUM_CLASSES)
