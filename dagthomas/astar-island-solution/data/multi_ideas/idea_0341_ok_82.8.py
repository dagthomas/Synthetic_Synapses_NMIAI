# Score: avg=82.81
# Direction: Increase the `port` class `global_multiplier` from 1.0 to 1.12 to conservatively address the underpr
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    prior_w = p.get("prior_w", 1.0) if isinstance(p, dict) else p["prior_w"]
    emp_max = p.get("emp_max", 5.0) if isinstance(p, dict) else p["emp_max"]
    ...
    probs[:, :, 2] = np.where(coastal_mask, probs[:, :, 2], 0.0)
    
    mountain_mask = (grid == 5)
    probs[:, :, 5] = np.where(mountain_mask, probs[:, :, 5], 0.0)
    
    probs = np.where(probs > 0, np.maximum(probs, 0.005), 0.0)
    
    row_sums = probs.sum(axis=-1, keepdims=True)
    probs = np.divide(probs, row_sums, out=np.zeros_like(probs), where=row_sums > 0)
    
    empty_rows = probs.sum(axis=-1) == 0
    if np.any(empty_rows):
        fallback = np.zeros(NUM_CLASSES)
        fallback[0] = 1.0
        probs[empty_rows] = fallback
