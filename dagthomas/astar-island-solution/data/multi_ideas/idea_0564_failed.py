# FAILED: Compilation failed: '[' was never closed (<string>, line 80)
# Direction: Increase the `port` class `global_calibr` from 83.0 to 86.5 to address the systematic underpredictio
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    global_calibr = np.array([1.0, 1.0, 86.5, 1.0, 1.0, 1.0])
    pred = pred * ratio * global_calibr

    if sim_pred is not None:
        pred = pred * (1.0 - sim_alpha) + sim_pred * sim_alpha

    if growth_front_map is not None:
        pred = pred * growth_front_map[:, :, np.newaxis]

    if obs_expansion_radius is not None and len(settlements) > 0:
        base_grid = np.ones((MAP_H, MAP_W), dtype=int)
        for s in settlements:
            if 0 <= s['row'] < MAP_H and 0 <= s['col'] < MAP_W:
                base_grid[s['row'], s['col']] = 0
        dist = distance_transform_cdt(base_grid, metric='taxicab')
        out_of_bounds = dist > obs_expansion_radius
        pred[out_of_bounds, 0] = 0.0
        pred[out_of_bounds, 1] = 0.0
        pred[out_of
    return probs