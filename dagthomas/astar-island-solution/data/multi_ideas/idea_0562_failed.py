...
# Direction: Increase the `settlement` class `clamp_min` from 0.01 to 0.04 to reduce the extreme KL divergence pe
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # 8. Constraints & Clamping
    # Nonzero floor
    pred[pred > 0] = np.maximum(pred[pred > 0], 0.005)
    
    # Settlement clamp (Increased from 0.01 to 0.04)
    pred[:, :, 1] = np.maximum(pred[:, :, 1], 0.04)
    return probs