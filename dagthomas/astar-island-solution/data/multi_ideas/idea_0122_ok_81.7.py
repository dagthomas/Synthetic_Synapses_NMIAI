# Score: avg=81.75
# Direction: Modify the settlement class coarse-level calibration maximum (coarse_max) from 2.0 to 1.85 to reduce
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    probs[:, :, 5] = np.where(grid == 4, probs[:, :, 5], 0.0)
    ...
    if obs_expansion_radius is not None and len(settlements) > 0:
        ...
        for r in range(MAP_H):
            for c in range(MAP_W):
                min_dist = min(abs(r - sr) + abs(c - sc) for sr, sc in settlements)
                if min_dist > obs_expansion_radius:
                    sett_mask[r, c] = False
        probs[~sett_mask, 1] = 0.0
