# Score: avg=83.59
# Direction: Increase the `port` class `global_multiplier` to 1.42 to specifically address coastal underpredictio
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    probs[mountain_mask, 5] = np.maximum(probs[mountain_mask, 5], 0.005)

    coastal_mask = _build_coastal_mask(grid)
    probs[~coastal_mask, 2] = 0.0
    probs[coastal_mask, 2] = np.maximum(probs[coastal_mask, 2], 0.005)
