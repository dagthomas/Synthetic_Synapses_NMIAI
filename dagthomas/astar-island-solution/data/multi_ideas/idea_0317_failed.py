# FAILED: Compilation failed: '[' was never closed (<string>, line 102)
# Direction: Increase the `settlement` class `global_multiplier` from 1.0 to 1.05 to address the dominant 4.5% KL
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # Increase the settlement class (1) global_multiplier from 1.0 to 1.05
    ratio[1] *= 1.05
    ...
    # Apply simulation predictions if available
    if sim_pred is not None:
        pred = pred * (1 - sim_alpha) + sim_pred * sim_alpha

    # Suppress settlement predictions beyond expansion radius
    if obs_expansion_radius is not None and len(settlements) > 0:
        ...
        for r, c in settlements:
            if 0 <= r < MAP_H and 0 <= c < MAP_W:
                sett_grid[r, c] = True
        if sett_grid.sum() > 0:
            dist = distance_transform_cdt(~sett_grid, metric='taxicab')
    ...
    # mountain(class 5) = 0 on non-mountain cells
    pred[:,
    return probs