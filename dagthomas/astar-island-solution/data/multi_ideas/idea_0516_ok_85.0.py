# Score: avg=85.04
# Direction: Increase the `settlement` class `clamp_m` upper bound from 2.5 to 2.65 to allow for higher peak prob
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    clamp_m = np.array([2.5, 2.5, 2.5, 2.65, 2.5, 2.5])
    ...
    if obs_expansion_radius is not None and settlements:
        ...
        pred[:, :, 3] = np.where(dist <= obs_expansion_radius, pred[:, :, 3], 0.0)
