# FAILED: Crashed on round2 seed 0: The truth value of an array with more than one element is ambiguous. Use a.any() or a.all()
# Direction: Increase the `settlement` class `coarse_max` calibration weight to 2.5 to better capture peak densit
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    cal_coarse_max = np.full(NUM_CLASSES, 2.0)
    cal_coarse_max[1] = 2.5
    
    cal_params = {
        'cal_fine_base': 1.0, 'cal_fine_divisor': 100.0, 'cal_fine_max': 5.0,
        'cal_coarse_base': 0.5, 'cal_coarse_divisor': 100.0, 'cal_coarse_max': cal_coarse_max,
    ...
    probs = np.where(pred_sum > 0, pred / pred_sum, 1.0 / NUM_CLASSES)
