# Score: avg=86.54
# Direction: Adjust the `settlement` class calibration clamp from [0.15, 2.5] to [0.17, 2.35] to tighten the pred
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    mult[1] = np.clip(mult[1], 0.17, 2.35)
    ...
    if is_sett.any():
        ...
        if obs_expansion_radius is not None:
            pred[dist_map > obs_expansion_radius, 1] = 0.0
    ...
    for c in range(NUM_CLASSES):
        if c == 2:
            pred[coastal, c] = np.maximum(pred[coastal, c], 0.005)
        elif c == 5:
            pred[is_mountain, c] = np.maximum(pred[is_mountain, c], 0.005)
        else:
            pred[:, :, c] = np.maximum(pred[:, :, c], 0.005)
