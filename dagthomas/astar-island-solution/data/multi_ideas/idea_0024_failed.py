# FAILED: Compilation failed: '(' was never closed (<string>, line 83)
# Direction: Increase the `settlement` class calibration `coarse_max` weight from 2.0 to 2.25. This conservative 
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    cal_params = {
        ...
        'cal_coarse_base': 0.5, 'cal_coarse_divisor': 100.0, 'cal_coarse_max': 2.25,
        ...
        for c in range(MAP_W):
            ...
            if t == 4:
                ...
                for i in range(NUM_CLASSES):
                    if i == 5:
                        continue
                    if i == 2 and not coastal[r, c]:
                        continue
                    val = pred[r, c, i]
                    if i == terrain_to_class[t]:
                        val += 0.1
                    if val > 0:
                        probs[r, c, i] = max(val, 0.005
    return probs