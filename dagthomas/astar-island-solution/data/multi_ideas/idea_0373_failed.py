# FAILED: Crashed on round2 seed 0: name 'pr' is not defined
# Direction: Adjust the `settlement` class `clamp_range` from [0.15, 2.5] to [0.13, 2.7] to reduce the KL diverge
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    priors = build_calibration_lookup(cal, unique_keys, his_params if 'his_params' in locals() else cal_params)
    ...
    class_mults = ratio.copy()
    class_mults[0] = np.clip(class_mults[0], 0.5, 2.0)
    class_mults[1] = np.clip(class_mults[1], 0.13, 2.7)
    class_mults[2] = np.clip(class_mults[2], 0.5, 2.0)
    class_mults[3] = np.clip(class_mults[3], 0.5, 2.0)
    class_mults[4] = np.clip(class_mults[4], 0.5, 2.0)
    class_mults[5] = np.clip(class_mults[5], 0.5, 2.0)
