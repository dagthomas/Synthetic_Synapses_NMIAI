# Score: avg=82.50
# Direction: Increase the `settlement` class `clamp_m` upper bound from 2.5 to 3.2 to reduce the suppression of h
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # 5. Multipliers & Clamping
    clamped_mults = np.ones(NUM_CLASSES)
    for c in range(NUM_CLASSES):
        if c == 1:
            clamped_mults[c] = np.clip(ratio[c], 0.1, 3.2)
        else:
            clamped_mults[c] = np.clip(ratio[c], 0.1, 2.5)
            
    pred = pred * clamped_mults[np.newaxis, np.newaxis, :]

    # 6. Simulated Prediction Overlay
