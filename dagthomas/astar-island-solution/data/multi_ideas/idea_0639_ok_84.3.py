# Score: avg=84.34
# Direction: Adjust the `settlement` class `clamp_m` range from its current bounds to [0.20, 2.20] to dampen the 
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    for c in range(NUM_CLASSES):
        m = ratio[c]
        if c == 1:
            clamp_m = np.clip(m, 0.20, 2.20)
        else:
            clamp_m = np.clip(m, 0.1, 5.0)
        class_mults[c] = clamp_m
