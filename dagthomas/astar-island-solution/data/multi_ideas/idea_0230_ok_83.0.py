# Score: avg=83.04
# Direction: Increase the `port` class global multiplier by 0.05 to address the coastal underprediction while mai
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    fallback = np.ones((MAP_H, MAP_W, NUM_CLASSES), dtype=float)
    fallback[~coastal_mask, 2] = 0.0
    fallback[~mountain_mask, 5] = 0.0
    fallback /= np.maximum(fallback.sum(axis=-1, keepdims=True), 1e-10)

    probs = np.where(row_sums > 1e-10, pred / np.maximum(row_sums, 1e-10), fallback)
