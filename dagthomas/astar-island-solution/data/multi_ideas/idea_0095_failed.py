...
# Direction: Adjust the `settlement` class probability clamp range from [0.15, 2.5] to [0.10, 2.8] to reduce over
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    priors = build_calibration_lookup(cal, unique_keys, Atkinsons_params)
    ...
    pred[:, :, 1] = np.clip(pred[:, :, 1], 0.10, 2.8)
    ...
    if obs_overlay is not None:
        obs_grid, obs_mask = obs_overlay
        pred[obs_mask] = obs_grid[obs_mask]
