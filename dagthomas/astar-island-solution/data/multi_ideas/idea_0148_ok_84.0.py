# Score: avg=84.01
# Direction: Adjust the `settlement` class `coarse_max` calibration weight from 2.0 to 2.15 to provide a more con
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    mountain_mask = np.zeros((MAP_H, MAP_W), dtype=bool)
    for r in range(MAP_H):
        for c in range(MAP_W):
            if terrain_to_class(grid[r, c]) == 5:
                mountain_mask[r, c] = True
