# FAILED: Compilation failed: '(' was never closed (<string>, line 104)
# Direction: Increase the `settlement` class `clamp_max` from 2.5 to 2.7 to allow for slightly higher peaks in hi
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # Clamp max (increased settlement clamp to 2.7)
    clamp_max = np.array([2.0, 2.7, 5.0, 5.0, 5.0, 5.0])
    ...
    for r in range(MAP_H):
        for c in range(MAP_W):
            for cls_idx in range(NUM_CLASSES):
                if pred[r, c, cls_idx] > 0:
                    pred[r, c, cls_idx] = max(pred[r, c, cls_idx], 0
    return probs