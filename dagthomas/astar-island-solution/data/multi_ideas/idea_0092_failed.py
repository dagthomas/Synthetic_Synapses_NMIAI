...
# Direction: Change the Feature-Key (FK) blend normalization from (pw + ew) to max(pw, ew) to prevent high-confid
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    norm_factor = np.maximum(prior_w, strengths)
    blended /= np.maximum(norm_factor[:, :, np.newaxis], 1e-10)
