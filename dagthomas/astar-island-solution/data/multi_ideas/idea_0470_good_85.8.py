# Score: avg=85.83
# Direction: Increase the `settlement` class `clamp_max` parameter from 2.5 to 2.75. This allows the model to bet
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # 5. Apply Ratio Multipliers
    clamp_max = np.array([1.5, 1.5, 2.0, 1.5, 2.75, 1.5])
    clamp_min = np.array([0.5, 0.5, 0.5, 0.5, 0.2, 0.5])
    mults = np.clip(ratio, clamp_min, clamp_max)
    pred = pred * mults
