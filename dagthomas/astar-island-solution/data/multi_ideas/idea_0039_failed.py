# FAILED: Compilation failed: invalid syntax (<string>, line 103)
# Direction: Adjust the `settlement` class probability clamp range from [0.15, 2.5] to [0.10, 3.0] to allow for h
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    if sim_pred is not None:
        pred = (1 - sim_alpha) * pred + sim_alpha * sim_pred
        pred /= np.maximum(pred.sum(axis=-1, keepdims=True), 1e-10)

    # Settlement clamp modification: adjusted to [0.10, 3.0]
    pred[:, :, 1] = np.clip(pred[:, :, 1], 0.10, 3.0)
