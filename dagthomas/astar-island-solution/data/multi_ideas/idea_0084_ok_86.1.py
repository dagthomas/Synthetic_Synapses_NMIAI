# Score: avg=86.12
# Direction: Increase the settlement class upper clamp from 2.5 to 2.75 to mitigate the dominant KL divergence er
# Error: None
...
def experimental_pred_fn(state: dict, global_mult,
    ...
    mult[2] = np.clip(mult[2], 0.15, 2.75)
    mult[3] = np.clip(mult[3], 0.15, 2.75)
    ...
    for c in range(NUM_CLASSES):
        valid = pred[:, :, c] > 0
        pred[valid, c] = np.maximum(pred[valid, c], 0.005)
