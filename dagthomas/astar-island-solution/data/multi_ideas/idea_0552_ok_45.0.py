# Score: avg=45.04
# Direction: Increase the `port` class `global_calibration` multiplier from 1.0 to 1.10 to mitigate coastal under
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    pred = uniform_filter(pred, size=(3, 3, 1), mode='constant')
    pred /= np.maximum(pred.sum(axis=-1, keepdims=True), 1e-10)
    ...
    if obs_overlay is not None:
        o_y, o_x, o_class = obs_overlay
        pred[o_y, o_x] = 0.0
        pred[o_y, o_x, o_class] = 1.0

    if sett_survival is not None:
        s_y, s_x, s_probs = sett_survival
        for y, x, pr in zip(s_y, s_x, s_probs):
            pred[y, x] = pr
