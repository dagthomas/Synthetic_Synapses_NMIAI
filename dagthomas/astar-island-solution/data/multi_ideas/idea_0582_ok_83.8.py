# Score: avg=83.79
# Direction: Adjust the `settlement` class `clamp_max` from 2.5 to 3.0 to reduce KL divergence by allowing the mo
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # 5. Global Multipliers with Clamp
    # Adjust settlement class (0) clamp_max from 2.5 to 3.0
    clamp_max = np.array([3.0, 2.5, 2.5, 2.5, 2.5, 2.5])
    probs *= np.minimum(ratio, clamp_max)
    ...
    try:
        mountain_terrains = [k for k, v in TERRAIN_TO_CLASS.items() if v == 5]
        is_mountain = np.isin(grid, mountain_terrains)
    except Exception:
        is_mountain = (grid == 3) | (grid == 4)
    probs[~is_mountain, 5] = 0.0

    # 7. Floor and Normalize
