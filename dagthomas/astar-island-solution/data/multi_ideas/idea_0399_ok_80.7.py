# Score: avg=80.67
# Direction: Increase the global prior multiplier for the `port` class from 1.0 to 1.08 to specifically address t
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # Apply 1.08 global prior multiplier for port class (index 2)
    ratio[2] *= 1.08
    ...
    smoothed = uniform_filter(pred, size=(3, 3, 1))
    pred = 0.8 * pred + 0.2 * smoothed

    is_coastal = _build_coastal_mask(grid)
    terrain_classes = np.vectorize(terrain_to_class)(grid)
    is_mountain = (terrain_classes == 5)

    pred[:, :, 5] = np.where(is_mountain, pred[:, :, 5], 0.0)
    pred[:, :, 2] = np.where(is_coastal, pred[:, :, 2], 0.0)

    pred = np.maximum(pred, 0.0)
    pred = np.where(pred > 0, np.maximum(pred, 0.005), 0.0)
