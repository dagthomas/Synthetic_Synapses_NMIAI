# Score: avg=86.43
# Direction: Increase the `coarse_max` calibration weight from 3.0 to 5.0 to give more influence to the most spec
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    if is_sett.any():
        ...
        decay = np.exp(-dist_map / 15.0)
        mult_grid = mult * (0.8 + 0.4 * decay[:, :, np.newaxis])
        pred *= mult_grid
    else:
        pred *= mult
    ...
    probs = pred / pred.sum(axis=-1, keepdims=True)
    return probs