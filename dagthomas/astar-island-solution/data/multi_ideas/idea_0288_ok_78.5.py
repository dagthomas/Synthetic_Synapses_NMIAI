# Score: avg=78.54
# Direction: Increase the `settlement` class `clamp_range` minimum from 0.05 to 0.10 to reduce extreme KL diverge
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # Blend simulation prediction
    if sim_pred is not None:
        pred = pred * (1.0 - sim_alpha) + sim_pred * sim_alpha

    # Base floors for all nonzero classes
    probs = np.maximum(pred, 0.005)
    
    # MODIFICATION: Increased settlement (class 4) clamp minimum to 0.10
    probs[..., 4] = np.maximum(probs[..., 4], 0.10)
