...
# Direction: Adjust the `settlement` class `clamp_range` from [0.05, 2.0] to [0.02, 2.2] to allow for higher prob
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    clamp_mins = np.array([0.05] * NUM_CLASSES)
    clamp_maxs = np.array([2.0] * NUM_CLASSES)
    clamp_mins[4] = 0.02
    return probs