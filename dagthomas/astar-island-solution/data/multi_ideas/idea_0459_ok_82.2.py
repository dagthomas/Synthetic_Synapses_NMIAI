# Score: avg=82.22
# Direction: Adjust the `settlement` class `coarse_max` calibration weight from 2.0 to 2.15 to slightly reduce th
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # 6. Spatial / Context (Sim/Growth Front)
    ...
    if obs_expansion_radius is not None:
        sett_mask = (np.array(settlements) == 1)
        if sett_mask.any():
            dist = distance_transform_cdt(~sett_mask, metric='taxicab')
            out_of_bounds = dist > obs_
    return probs