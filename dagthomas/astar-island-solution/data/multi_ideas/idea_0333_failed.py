# FAILED: Compilation failed: '[' was never closed (<string>, line 57)
# Direction: Adjust the `settlement` class `clamp_range` from `[0.15, 2.5]` to `[0.10, 2.7]` to allow for more ag
...
def experimental_pred_fn(state: dict, global_mult,
    ...
    clamped_ratio = np.clip(ratio, 0.1, 10.0)
    clamped_ratio[1] = np.clip(ratio[1], 0.10, 2.7)
