# Score: avg=82.65
# Direction: Increase the `settlement` class `global_multiplier` from 1.0 to 1.07 to reduce the dominant KL diver
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    for r in range(MAP_H):
        for c in range(MAP_W):
            terrain = grid[r, c]
            t_class = terrain_to_class(terrain) if callable(terrain_to_class) else terrain_to_class[terrain]
            if t_class != 5:
                pred[r, c, 5] = 0.0
            if not coastal[r, c]:
                pred[r, c, 2] = 0.0
