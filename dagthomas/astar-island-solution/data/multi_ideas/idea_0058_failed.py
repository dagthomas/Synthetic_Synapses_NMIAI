...
# Direction: Increase the `port` class global multiplier from 1.0 to 1.18 to address the systematic underpredicti
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    """
    p = globals().get('_load_params', lambda: {"prior_w": 2.0, "emp_max": 3.0})()
    ...
    if obs_expansion_radius is not None and len(settlements) > 0:
        ...
        pred[:, :, 1] = np.where(dist > obs_expansion_radius, 0.0, pred[:, :, 1])

    pred = np.where(pred > 0, np.maximum(pred, 0.005), 0.0)
    
    row_sums = pred.sum(axis=-1, keepdims=True)
    probs = np.where(row_sums > 0, pred
    return probs