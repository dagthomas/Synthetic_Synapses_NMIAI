# Score: avg=84.34
# Direction: Adjust the `settlement` class `clamp_range` from `[0.15, 2.5]` to `[0.17, 2.4]` to reduce entropy-we
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # Apply narrowed clamp range to settlement class (class 1)
    ratio[1] = np.clip(ratio[1], 0.17, 2.4)
    ...
    probs = np.where(s > 0, pred / s, 0.0)
    probs[s.squeeze(-1) == 0, 0] = 1.0
