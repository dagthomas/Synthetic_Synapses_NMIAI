# FAILED: Compilation failed: invalid syntax (<string>, line 101)
# Direction: Increase the `settlement` class `clamp_m` upper bound from 2.5 to 2.85 to allow for more aggressive 
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # 5. Apply Global Multipliers
    pred *= ratio

    # 6. Clamping with modified upper bound for settlement
    clamp_m = np.array([2.5, 2.85, 2.5, 2.5, 2.5, 2.5])
    clamp_min = 0.05
    raw_p = raw_priors[idx_grid]
    pred = np.clip(pred, raw_p * clamp_min, raw_p * clamp_m)

    # 7. Sim Prediction Blending
    if sim_pred is not None:
        pred = pred * (1 - sim_alpha) + sim_pred * sim_alpha

    # 8. Spatial Rules & Constraints
