# Score: avg=85.66
# Direction: Increase the `port` class global multiplier from 1.0 to 1.10 to mitigate coastal underprediction. Th
# Error: None
...
def experimental_pred_fn(state: dict, global_mult,
    ...
    p = {"prior_w": 1.0, "emp_max": 5.0, "ratio_w": 0.5}
    ...
    probs = probs * (ratio ** p.get("ratio_w", 0.5))
    ...
    mask = probs > 0
    probs[mask] = np.maximum(probs[mask], 0.005)
