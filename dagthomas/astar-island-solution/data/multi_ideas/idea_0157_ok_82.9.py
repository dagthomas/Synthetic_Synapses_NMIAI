# Score: avg=82.90
# Direction: Adjust the `settlement` class `coarse_max` multiplier from 2.0 to 1.9 to move the calibration closer
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    cal_params = {
        ...
        'cal_coarse_base': 0.5, 'cal_coarse_divisor': 100.0, 'cal_coarse_max': 1.9,
    ...
    if obs_expansion_radius is not None:
        ...
        if not np.any(sett_mask):
            sett_mask[MAP_H//2, MAP_W//2] = True
    ...
    for r in range(MAP_H):
        for c in range(MAP_W):
            terrain_cls = terrain_to_class(grid[r, c])
            if terrain_cls != 5:
                pred[r, c, 5] = 0.0
            if not coast[r, c]:
                pred[r, c, 2] = 0.0
