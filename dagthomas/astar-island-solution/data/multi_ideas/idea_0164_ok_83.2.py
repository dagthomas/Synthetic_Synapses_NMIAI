# Score: avg=83.17
# Direction: Increase the `settlement` class `coarse_max` calibration weight from 2.0 to 2.15 to tighten the prob
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    for r in range(MAP_H):
        for c in range(MAP_W):
            terr_cls = terrain_to_class(grid[r, c])
            if terr_cls != 5:
                pred[r, c, 5] = 0.0
    ...
    zero_sums = (row_sums.squeeze() == 0)
    if np.any(zero_sums):
        for r in range(MAP_H):
            for c in range(MAP_W):
                if zero_sums[r, c]:
                    valid = np.ones(NUM_CLASSES)
                    if terrain_to_class(grid[r, c]) != 5:
                        valid[5] = 0.0
                    if not coastal_mask[r, c]:
                        valid[2] = 0.0
                    probs[r, c] = valid / valid.sum()
