# Score: avg=82.98
# Direction: Increase the `port` class global multiplier from 1.0 to 1.12 to address the underprediction in coast
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # Apply ratio and port global multiplier modification
    ratio[2] *= 1.12
    pred *= ratio
    ...
    for i in range(MAP_H):
        for j in range(MAP_W):
            terrain_classes[i, j] = terrain_to_class(grid[i, j])
