# Score: avg=83.75
# Direction: Increase the `settlement` class `clamp_m` upper bound from 2.5 to 2.75 to capture more high-probabil
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    clamp_m = np.clip(ratio, 0.5, 2.0)
    clamp_m[1] = np.clip(ratio[1], 0.5, 2.75)
    pred = pred * clamp_m
    ...
    if growth_front_map is not None:
        pred[:, :, 1] *= np.maximum(growth_front_map, 0.1)
    ...
    pred = np.where(pred > 0, np.maximum(pred, 0.005), 0)
