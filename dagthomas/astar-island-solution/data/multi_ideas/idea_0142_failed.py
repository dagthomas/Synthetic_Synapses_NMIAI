# FAILED: Crashed on round2 seed 0: 'int' object is not iterable
# Direction: Adjust the `settlement` class `coarse_max` calibration weight from 2.0 to 1.85 to reduce the impact 
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    cal_params = {
        ...
        'cal_coarse_base': 0.5, 'cal_coarse_divisor': 100.0, 'cal_coarse_max': 1.85,
    ...
    for r in range(MAP_H):
        for c in range(MAP_W):
            terrain = grid[r, c]
            valid_classes = terrain_to_class(terrain)
            
            for cls in valid_classes:
                probs[r, c, cls] = pred[r, c, cls]
                
            if 2 in valid_classes and not coastal_mask[r, c]:
                probs[r, c, 2] = 0.0
            if 5 not in valid_classes:
                probs[r, c, 5] = 0.0
            ...
            nonzero = probs[r, c] > 0
            probs[r, c, nonzero] = np.maximum(probs[r, c, nonzero], 0.005)
