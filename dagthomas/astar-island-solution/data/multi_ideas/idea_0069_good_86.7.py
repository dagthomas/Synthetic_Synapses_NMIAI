# Score: avg=86.67
# Direction: Increase the `coarse_max` calibration weight from 2.0 to 2.5 to improve the anchor of settlement pro
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    p = {"prior_w": 2.0, "emp_max": 3.0, "base_power": 0.5, "spatial_smooth": 0.1}
    ...
    if obs_expansion_radius is not None:
        suppress_mask = dist_map > obs_expansion_radius
        pred[suppress_mask, 1:4] *= 0.1

    sm = p["spatial_smooth"]
    if sm > 0:
        smoothed = uniform_filter(pred, size=3, mode='constant')
        pred = pred * (1 - sm) + smoothed * sm

    coast = _build_coastal_mask(grid)
    pred[~coast, 2] = 0.0
    pred[grid != 3, 5] = 0.0
