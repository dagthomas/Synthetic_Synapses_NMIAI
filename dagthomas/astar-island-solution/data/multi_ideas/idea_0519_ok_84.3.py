# Score: avg=84.27
# Direction: Increase the `port` class global calibration multiplier by 0.12 to mitigate coastal underprediction 
# Error: None
...
def experimental_pred_fn(state: dict, global_mult,
    ...
    ratio[2] += 0.12
    ratio[1] = np.clip(ratio[1], 0.18, 2.85)
    ...
    allowed = np.ones_like(pred, dtype=bool)
    allowed[~coastal, 2] = False
    allowed[~is_mountain, 5] = False

    pred[allowed] = np.maximum(pred[allowed], 0.005)
