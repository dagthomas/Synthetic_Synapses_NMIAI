# FAILED: Compilation failed: expected ':' (<string>, line 99)
# Direction: Increase the `settlement` class `clamp_max` from 2.5 to 2.8 to allow empirical feature-key data to m
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # Increase the settlement class (index 3) clamp_max from 2.5 to 2.8
    clamp_max = np.array([2.5, 2.5, 2.5, 2.8, 2.5, 2.5])
    clamp_min = np.array([0.4, 0.4, 0.4, 0.4, 0.4, 0.4])
    r_clamped = np.clip(ratio, clamp_min, clamp_max)
    pred = pred * r_clamped

    # 6. Filters and Constraints
    coastal = _build_coastal_mask(grid)
    is_mountain = (grid == 5)
    
    if obs_expansion_radius is not None and obs_expansion_radius > 0:
        is_init_sett = (np.array(settlements) == 1)
        dist = distance_transform_cdt(~is_init_sett, metric='taxicab')
    ...
    if growth_front_map is not None:
        pred[:, :, 3] *= np.maximum(0.1, growth_front_map)
