# Score: avg=80.99
# Direction: Adjust the `settlement` class `clamp_range` from [0.15, 2.5] to [0.20, 2.2] to reduce the high entro
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    pred[:, :, 1] = np.clip(pred[:, :, 1],
                            raw_priors[idx_grid][:, :, 1] * 0.20,
                            raw_priors[idx_grid][:, :, 1] * 2.2)
    ...
    if obs_expansion_radius is not None and len(settlements) > 0:
        coords = np.array(settlements)
        mask = np.ones((MAP_H, MAP_W), dtype=bool)
        mask[coords[:, 0], coords[:, 1]] = False
