# Score: avg=81.58
# Direction: Increase the `port` class `global_multiplier` from 1.1 to 1.25 to compensate for the identified coas
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # 5. Apply Multipliers
    pred = pred * ratio
    pred[:, :, 2] *= 1.25  # Increased port class multiplier
    ...
    probs = np.where(row_sums > 0, pred / np.maximum(row_sums, 1e-10), 0.0)
    
    zero_mask = probs.sum(axis=-1) == 0
    if np.any(zero_mask):
