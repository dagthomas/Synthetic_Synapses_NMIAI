# Score: avg=78.87
# Direction: Increase the `settlement` class `clamp_min` parameter from 0.15 to 0.18 to prevent the Bayesian prio
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    pred[:, :, 1] = np.maximum(pred[:, :, 1], 0.18)

    pred[:, :, 5] = np.where(grid == 2, pred[:, :, 5], 0.0)
