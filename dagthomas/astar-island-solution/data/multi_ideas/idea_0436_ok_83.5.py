# Score: avg=83.52
# Direction: Adjust the `settlement` class `clamp_range` from `[0.15, 2.5]` to `[0.12, 2.75]` to allow for slight
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    for c in range(NUM_CLASSES):
        r = np.clip(ratio[c], clamp_ranges[c][0], clamp_ranges[c][1])
        pred[:, :, c] *= r
    ...
    for r in range(MAP_H):
        for c in range(MAP_W):
            ...
            for cls in range(NUM_CLASSES):
                if pred[r, c, cls] > 0:
                    pred[r, c, cls] = max(pred[r, c, cls], 0.005)
