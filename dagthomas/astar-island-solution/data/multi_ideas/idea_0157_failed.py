# FAILED: Compilation failed: '(' was never closed (<string>, line 86)
# Direction: Increase the global multiplier for the `port` class from 1.0 to 1.10 to address coastal underpredict
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    mult[2] = np.clip(mult[2], 0.15, 2.0) * 1.10
    ...
    if is_sett.any():
        ...
        dist_map = np.full_like(grid, 100)
    ...
    pred[:, :, 0] = np.maximum(pred[:, :, 0], 0.005)
    pred[:, :, 1] = np.maximum(pred
    return probs