# Score: avg=83.78
# Direction: Increase the `settlement` class `clamp_max` from 2.5 to 3.0 to better capture high-density settlemen
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    try:
        ...
        p = {"prior_w": 1.0, "emp_max": 2.0, "blur_sigma": 0.1}
    ...
    clamp_max = np.array([2.5, 3.0, 2.5, 2.5, 2.5, 2.5])
    ...
    if p.get("blur_sigma", 0) > 0:
        ...
        pred = pred * (1 - p.get("blur_sigma", 0)) + smoothed * p.get("blur_sigma", 0)
