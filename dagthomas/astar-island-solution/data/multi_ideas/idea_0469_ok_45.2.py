# Score: avg=45.21
# Direction: Adjust the `settlement` class `clamp_max` from its current value to 2.2 to prevent outlier buckets f
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    mults = ratio.copy()
    mults[0] = np.clip(mults[0], 0.5, 2.0)
    mults[1] = np.clip(mults[1], 0.1, 2.2)
    mults[2] = np.clip(mults[2], 0.1, 4.0)
    mults[3] = np.clip(mults[3], 0.1, 4.0)
    mults[4] = np.clip(mults[4], 0.1, 4.0)
    mults[5] = np.clip(mults[5], 0.1, 4.0)

    pred *= mults
    pred /= np.maximum(pred.sum(axis=-1, keepdims=True), 1e-10)

    pred = uniform_filter(pred, size=(3, 3, 1), mode='reflect')
