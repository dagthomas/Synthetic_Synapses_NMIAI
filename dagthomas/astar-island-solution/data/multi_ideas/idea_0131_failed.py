# FAILED: Compilation failed: unterminated string literal (detected at line 23) (<string>, line 23)
# Direction: Change the FK blend normalization formula from `(pw + ew)` to `max(pw, ew)` to prevent high-confiden

def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
                   fk_buckets: FeatureKeyBuckets,
                   multi_store=None,
                   variance_regime: str = None,
                   obs_expansion_radius: int = None) -> np.ndarray:
    """Production prediction with auto-loaded best params."""
    # Fallback parameters if _load_params is not in scope
    p = {"prior_w": 1.0, "emp_max": 5.0, "base_power": 1.0}
    if 'predict' in globals() and hasattr(predict, '_load_params'):
        p = predict._load_params()
        
    grid = np.array(state['grid'])
    settlements = state['settlements']

    # 1. Feature Keys
    fkeys = build_feature_keys(grid, settlements)
    idx_grid, unique_keys = _build_feature_key_index(fkeys)

    # 2. Calibration
    cal = predict.get_calibration()
    cal_params = {
        'cal_fine_base': 1.0, 'cal_fine_divisor': 100.0, 'cal_fine_max': 5.0,
        'cal_coarse_base': 0.5, 'cal_coarse_divisor': 100.0, 'cal_coarse_
    return probs