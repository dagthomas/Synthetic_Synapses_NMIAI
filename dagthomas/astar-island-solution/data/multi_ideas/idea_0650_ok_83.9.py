# Score: avg=83.89
# Direction: Increase the `settlement` class `clamp_m` upper bound from 2.5 to 2.8 to allow for higher peak proba
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    clamp_m = np.clip(ratio, 0.1, 5.0)
    clamp_m[1] = np.clip(ratio[1], 0.1, 2.8)
    probs *= clamp_m
