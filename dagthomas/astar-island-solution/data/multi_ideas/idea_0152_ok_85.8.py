# Score: avg=85.78
# Direction: Change the FK blend normalization from `(pw + ew)` to `max(pw, ew)` to prevent strong feature-key si
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # MODIFICATION: Changed normalization from (pw + ew) to max(pw, ew)
    norm = np.maximum(prior_w, strengths)[:, :, np.newaxis]
    blended = (pred * prior_w + emp_grid * strengths[:, :, np.newaxis]) / norm
    ...
    probs = pred * mult[np.newaxis, np.newaxis, :]
    ...
    if is_sett.any():
        ...
        if obs_expansion_radius is not None:
            ...
            probs[out_of_bounds, 1:4] *= 0.01
    else:
        dist_map = np.full((MAP_H, MAP_W), 50.0)

    # Apply constraints and floor
