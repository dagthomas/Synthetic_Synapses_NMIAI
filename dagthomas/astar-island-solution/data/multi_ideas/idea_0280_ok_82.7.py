# Score: avg=82.71
# Direction: Increase the `port` class `global_multiplier` from 1.0 to 1.05 to address coastal underprediction wi
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # MODIFICATION: Increase port class global_multiplier from 1.0 to 1.05
    ratio[2] *= 1.05
    ...
    allowed = np.ones((MAP_H, MAP_W, NUM_CLASSES), dtype=bool)
    allowed[~is_mountain, 5] = False
    allowed[~coastal_mask, 2] = False

    pred[~allowed] = 0.0

    # Apply floor >= 0.005 for nonzero classes
    pred = np.where(allowed & (pred < 0.005), 0.005, pred)

    # Normalize so all rows sum to 1.0
