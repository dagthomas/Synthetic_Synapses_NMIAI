# Score: avg=82.78
# Direction: Increase the `settlement` class `clamp_max` from 2.5 to 2.7 to allow for higher-confidence peaks in 
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    try:
        ...
        p = {"prior_w": 1.5, "emp_max": 3.0, "smooth_w": 0.1}
    ...
    for c in range(NUM_CLASSES):
        sm = uniform_filter(pred[:, :, c], size=3)
        pred[:, :, c] = pred[:, :, c] * (1 - smooth_w) + sm * smooth_w

    clamp_max = np.array([10.0, 10.0, 10.0, 2.7, 10.0, 10.0])
