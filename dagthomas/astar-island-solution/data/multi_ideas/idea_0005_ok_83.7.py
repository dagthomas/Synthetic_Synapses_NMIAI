# Score: avg=83.75
# Direction: Reduce the settlement prior multiplier by 10%—settlement dominates your KL error (4.5%), and since
# Error: None
...
def experimental_pred_fn(state, global_mult, fk_buckets):
    ...
    cal_params = {
        ...
        'cal_coarse_base': 0.5, 'cal_coarse_divisor': 100.0, 'cal_coarse_max': 1.8,
    ...
    # Settlement prior boost reduced by 10% to combat over-confidence
    sett_prior_boost = 0.9 * (1.30 + 0.17 * (regime_raw - 0.3) / 1.2)
    ...
    for i in range(N):
        ...
        mults[1] = np.clip(mults[1], 0.08, 2.5)
        mults[2] = np.clip(mults[2], 0.12, 2.5)
        mults[3] = np.clip(mults[3], 0.12, 2.5)
    ...
    # 5b. Empty class: inverse sigmoid
    ...
    port_prior = 0.05
    port_mult = 1.0 + 1.5 * np.exp(-0.5 * (dist_to_coast / 2.0) ** 2)
    port_mult[~land_mask] = 1.0
