# FAILED: Crashed on round2 seed 0: 'function' object has no attribute 'get'
# Direction: Increase the global multiplier for the `port` class from 1.0 to 1.1 to address the consistent underp
...
def experimental_pred_fn(state: dict, global_mult,
    ...
    if obs_expansion_radius is not None and len(settlements) > 0:
        sett_locs = np.array([(s['y'], s['x']) for s in settlements])
        y_coords, x_coords = np.indices((MAP_H, MAP_W))
        dist = np.min(np.sqrt((y_coords[:, :, np.newaxis] - sett_locs[:, 0])**2 + 
                              (x_coords[:, :, np.newaxis] - sett_locs[:, 1])**2), axis=2)
    ...
    for y in range(MAP_H):
        for x in range(MAP_W):
            terrain = grid[y, x]
            c = terrain_to_class.get(terrain, -1)
            if c != -1:
                probs[y, x] = 0.0
                probs[y, x, c] = 1.0

    sums = probs.sum(axis=-1, keepdims=True)
    probs = np.where(sums > 0, probs / sums, 0.0)
    zero_mask = sums.squeeze(-1) == 0
    probs[zero_mask, 0] = 1.0
