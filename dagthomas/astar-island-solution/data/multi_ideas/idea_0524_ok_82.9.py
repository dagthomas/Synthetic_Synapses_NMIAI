# Score: avg=82.86
# Direction: Increase the `port` class global calibration multiplier from 1.0 to 1.1 to compensate for the identi
# Error: None
    ...
    for r in range(MAP_H):
        for c in range(MAP_W):
            ...
            for i in range(NUM_CLASSES):
                if p_cell[i] > 0:
                    p_cell[i] = max(p_cell[i], 0.005)
                    
            s = p_cell.sum()
            if s > 0:
                p_cell /= s
            else:
                p_cell[base_class] = 1.0
