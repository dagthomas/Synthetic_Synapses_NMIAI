...
# Direction: Adjust the `settlement` class clamp range from [0.15, 2.5] to [0.12, 2.8] to better capture high-pro
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    try:
        ...
        p = {"prior_w": 1.5, "emp_max": 3.0, "smooth_size": 3}
    ...
    if smooth_size > 1:
        for c in range(NUM_CLASSES):
            pred[:, :, c] = uniform_filter(pred[:, :, c], size=smooth_size)
        pred /= np.maximum(pred.sum(axis=-1, keepdims=True), 1e-10)

    # Settlement clamp (Adjusted from [0.15, 2.5] to [0.12, 2.8])
    pred[:, :, 0] = np.clip(pred[:, :, 0], 0.12, 2.8)

    # Masks
    coastal_mask = _build_coastal_mask(grid)
    pred[:, :, 2] *= coastal_mask

    is_mountain = (grid == 4) | (grid == 5)
    pred[:, :, 5] *= is_mountain

    # Floor and re-apply masks
