# Score: avg=82.92
# Direction: Increase the `settlement` class `clamp_max` value from 2.5 to 2.8 to allow for higher peak probabili
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    clamp_max_arr = np.array([2.5, 2.5, 2.5, 2.8, 2.5, 2.5])
    pred = np.minimum(pred, clamp_max_arr)
    pred /= np.maximum(pred.sum(axis=-1, keepdims=True), 1e-10)
