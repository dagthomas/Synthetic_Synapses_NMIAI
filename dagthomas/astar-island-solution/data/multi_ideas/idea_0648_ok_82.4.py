# Score: avg=82.42
# Direction: Adjust the FK blend normalization formula from `(pw + ew)` to `max(pw, ew)` to prevent the dilution 
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    norm_factor = np.maximum(prior_w, ew)
    blended = (pred * prior_w + emp_grid * ew) / norm_factor
    ...
    mountain_mask = (grid == 4) | (grid == 5)

    probs[~mountain_mask, 5] = 0.0
    probs[~coastal_mask, 2] = 0.0

    valid_mask = np.ones((MAP_H, MAP_W, NUM_CLASSES), dtype=bool)
    valid_mask[~mountain_mask, 5] = False
    valid_mask[~coastal_mask, 2] = False

    probs = np.where(valid_mask, np.maximum(probs, 0.005), 0.0)
