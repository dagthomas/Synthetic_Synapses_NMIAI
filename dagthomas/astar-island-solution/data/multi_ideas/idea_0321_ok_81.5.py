# Score: avg=81.46
# Direction: Increase the `coarse_max` calibration weight to 5.0 to further stabilize the high-level settlement d
# Error: None
    ...
    for r in range(MAP_H):
        for c in range(MAP_W):
            ...
            if isinstance(terrain_to_class, dict):
                t_class = terrain_to_class.get(terrain, 0)
            elif hasattr(terrain_to_class, '__getitem__'):
            ...
            mask = cell_probs > 0
            cell_probs[mask] = np.maximum(cell_probs[mask], 0.005)
            
            s = cell_probs.sum()
            if s > 0:
                probs[r, c] = cell_probs / s
            else:
                probs[r, c, 0] = 1.0
