# FAILED: Compilation failed: invalid syntax (<string>, line 46)
# Direction: Increase the settlement class calibration weight from 1.0 to 1.05 to mitigate its high KL divergence
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # Increase settlement class calibration weight to mitigate high KL divergence
    priors[:, 1] *= 1.05
    priors /= np.maximum(priors.sum(axis=-1, keepdims=True), 1e-10)
    ...
    ratio = <strike>obs / exp_arr</strike>
    ratio = obs / exp_arr
    ...
    if is_sett.any():
        ...
        dist_mult = 1.0 / (1.0 + dist_map)
        pred[:, :, 1] *= np.power(dist_mult, 0.5)
        
        if obs_expansion_radius is not None:
            beyond_radius = dist_map > obs_expansion_radius
            pred[beyond_radius, 1] *= 0.1
