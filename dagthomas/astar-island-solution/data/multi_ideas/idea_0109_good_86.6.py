# Score: avg=86.56
# Direction: Increase the `port` class global multiplier from 1.0 to 1.07 to address coastal underprediction.
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    for i in range(MAP_H):
        for j in range(MAP_W):
            ...
            c_probs = pred[i, j].copy()
            c_probs[5] = 0.0
            if tc == 5:
                c_probs = np.zeros(NUM_CLASSES)
                c_probs[5] = 1.0
            else:
                if not coastal[i, j]:
                    c_probs[2] = 0.0
                c_probs = np.maximum(c_probs, 0.005)
                if not coastal[i, j]:
                    c_probs[2] = 0.0
                c_probs[5] = 0.0
                s = c_probs.sum()
