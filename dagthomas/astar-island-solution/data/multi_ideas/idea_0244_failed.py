# FAILED: Compilation failed: invalid syntax (<string>, line 79)
# Direction: Increase the `settlement` class `coarse_max` calibration weight to 2.2 to better regularize the domi
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    for r in range(MAP_H):
        for c in range(MAP_W):
            terrain = grid[r, c]
            t_cls = terrain_to_class[terrain] if isinstance(terrain_to_class, (dict, list, tuple)) else terrain_to_class(terrain)
