# FAILED: Compilation failed: '(' was never closed (<string>, line 92)
# Direction: Adjust the `settlement` class `clamp_range` from [0.15, 2.5] to [0.20, 2.20] to reduce the impact of
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # 5. Global Ratio Application with adjusted clamp_range for settlement
    clamp_min = np.array([0.5, 0.5, 0.5, 0.5, 0.20, 0.5])
    clamp_max = np.array([2.0, 2.0, 2.0, 2.0, 2.20, 2.0])
    ...
    if obs_expansion_radius is not None and len(settlements) > 0:
        ...
        pred[dist > obs_expansion_radius, 4] *= 0.01
        pred /= np.maximum(pred.sum(axis=-1, keepdims=True
    return probs