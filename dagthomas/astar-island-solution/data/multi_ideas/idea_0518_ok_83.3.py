# Score: avg=83.29
# Direction: Increase the `settlement` class `clamp_m` range from [0.15, 2.5] to [0.10, 3.25] to allow for higher
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    clamp_m[0] = np.clip(ratio[0], 0.2, 5.0)
    clamp_m[1] = np.clip(ratio[1], 0.10, 3.25)
    clamp_m[2] = np.clip(ratio[2], 0.2, 5.0)
    clamp_m[3] = np.clip(ratio[3], 0.2, 5.0)
    clamp_m[4] = np.clip(ratio[4], 0.2, 5.0)
    clamp_m[5] = np.clip(ratio[5], 0.2, 5.0)
