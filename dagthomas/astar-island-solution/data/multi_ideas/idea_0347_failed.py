...
# Direction: Adjust the `settlement` class `clamp_range` from [0.15, 2.5] to [0.12, 2.8] to reduce the KL penalty
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    for c in range(NUM_CLASSES):
        if c in clamp_ranges:
            low, high = clamp_ranges[c]
            pred[:, :, c] = np.clip(pred[:, :, c], raw_priors[idx_grid][:, :, c] * low, raw_priors[idx_grid][:, :, c] * high)

    if sett_survival is not None:
        sr_mult, sr_mask = sett_survival
        pred[sr_mask, 1] *= sr_mult
        
    if obs_overlay is not None:
        obs_mult, obs_mask = obs_overlay
        for c in range(NUM_CLASSES):
            pred[obs_mask, c] *= obs_mult[obs_mask, c]

    if sim_pred is not None:
        pred = pred * (1 - sim_alpha) + sim_pred * sim_alpha
        
    if growth_front_map is not None:
        pred[:, :, 1] *= growth_front_map
    ...
    if obs_expansion_radius is not None and len(settlements) > 0:
        ...
        if sett_mask.any():
            ...
            pred[suppress_mask, 1] *= 0.01

    coastal = _build_coastal_mask(grid)
    pred[:, :, 2] *= coastal
    
    is_mountain = (grid == 5)
    pred[:, :, 5] = is_mountain.astype(float)
    pr
    return probs