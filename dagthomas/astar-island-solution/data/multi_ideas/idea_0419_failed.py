# FAILED: Compilation failed: '(' was never closed (<string>, line 75)
# Direction: Increase the `settlement` class `clamp_range` upper bound from 2.5 to 3.0 to allow for more confiden
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    ratio[0] = np.clip(ratio[0], 0.1, 2.5)
    ratio[1] = np.clip(ratio[1], 0.1, 3.0)
    ratio[2] = np.clip(ratio[2], 0.1, 2.5)
    ratio[3] = np.clip(ratio[3], 0.1, 2.5)
    ratio[4] = np.clip(ratio[4], 0.1, 2.5)
    ratio[5] = np.clip(ratio[5], 0.1, 2.5)
