# Score: avg=82.76
# Direction: Adjust the `settlement` class `coarse_max` calibration weight to 2.25 to stabilize the dominant KL e
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    if obs_expansion_radius is not None and len(settlements) > 0:
        sett_r, sett_c = zip(*settlements)
        mask = np.ones((MAP_H, MAP_W), dtype=bool)
        mask[sett_r, sett_c] = False
