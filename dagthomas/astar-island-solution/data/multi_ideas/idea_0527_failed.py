...
# Direction: Adjust the `settlement` class `clamp_max` from 2.5 to 2.65 to allow for slightly more confident pred
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # 5. Multipliers with targeted settlement clamp adjustment (2.5 -> 2.65)
    clamp_max = np.array([2.0, 2.65, 2.0, 2.0, 2.0, 1.0])
    clamp_min = np.array([0.5, 0.5, 0.5, 0.5, 0.5, 1.0])
    ratio_clamped = np.clip(ratio, clamp_min, clamp_max)
