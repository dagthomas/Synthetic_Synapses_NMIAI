# Score: avg=79.41
# Direction: Adjust the settlement class `clamp_range` from [0.15, 2.5] to [0.12, 3.2] to allow for sharper proba
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    pred[:, :, 1] = np.clip(pred[:, :, 1], 0.12, 3.2)

    sm_pred = uniform_filter(pred, size=(3, 3, 1))
    pred = 0.8 * pred + 0.2 * sm_pred
    ...
    if sim_pred is not None:
        probs = (1.0 - sim_alpha) * probs + sim_alpha * sim_pred
