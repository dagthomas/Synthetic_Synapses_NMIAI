# Score: avg=84.62
# Direction: Increase the `settlement` class `clamp_m` upper bound from 2.5 to 2.75. This slightly raises the inf
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # 5. Global Multipliers with specific class bounds
    m = np.clip(ratio, 0.5, 2.0)
    m[0] = np.clip(ratio[0], 0.5, 1.5)
    m[3] = np.clip(ratio[3], 0.5, 2.75)
    m[5] = np.clip(ratio[5], 0.5, 1.5)
    pred *= m
