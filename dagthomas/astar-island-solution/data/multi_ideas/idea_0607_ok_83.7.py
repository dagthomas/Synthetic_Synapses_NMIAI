# Score: avg=83.73
# Direction: Increase the `settlement` class `clamp_max` from 2.5 to 3.0 to allow the Bayesian update to better c
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    ratio_clamped = np.clip(ratio, 0.1, 2.5)
    ratio_clamped[3] = np.clip(ratio[3], 0.1, 3.0)
    
    probs = pred * ratio_clamped
