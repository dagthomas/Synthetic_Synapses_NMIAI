# FAILED: Compilation failed: invalid syntax (<string>, line 58)
# Direction: Increase the `port` class global multiplier from 1.0 to 1.08 to mitigate coastal underprediction wit
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    if est_vigor is not None and hasattr(predict, 'get_regime_calibration'):
        cal = predict.get_regime_calibration(est_vigor)
    elif hasattr(predict, 'get_calibration'):
    ...
    # Ratio
    if global_mult is not None and hasattr(global_mult, 'observed') and global_mult.observed.sum() >
    return probs