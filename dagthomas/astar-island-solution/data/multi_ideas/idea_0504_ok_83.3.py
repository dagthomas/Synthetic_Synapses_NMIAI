# Score: avg=83.25
# Direction: Increase the `settlement` class `clamp_max` from 2.5 to 2.65 to provide a marginal headroom increase
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    clamp_maxes = np.full(NUM_CLASSES, 2.5)
    clamp_maxes[3] = 2.65
    ratio_clamped = np.clip(ratio, 0.1, clamp_maxes)
    pred *= ratio_clamped
