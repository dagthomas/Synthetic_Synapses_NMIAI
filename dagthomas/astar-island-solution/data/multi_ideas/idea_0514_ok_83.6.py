# Score: avg=83.61
# Direction: Increase the `settlement` class `clamp_max` from 2.5 to 2.8 to allow for higher peak probabilities i
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # Clamp modifications (Settlement max increased to 2.8)
    clamp_max = np.array([1.5, 2.8, 2.0, 1.5, 1.5, 1.0])
    pred = np.minimum(pred, clamp_max)
    ...
    pred[:, :, 2] = np.where(~coastal & ~is_mountain, 0.0, pred[:, :, 2])

    # Floor
    pred = np.maximum(pred, 0.005)

    # Re-apply strict masks after floor
    pred[:, :, 5] = np.where(is_mountain, pred[:, :, 5], 0.0)
    for c in range(NUM_CLASSES):
        if c != 5:
            pred[:, :, c] = np.where(is_mountain, 0.0, pred[:, :, c])
    ...
    if sim_pred is not None:
        ...
        for c in range(NUM_CLASSES):
            if c != 5:
                probs[:, :, c] = np.where(is_mountain, 0.0, probs[:, :, c])
        pro
    return probs