# Score: avg=83.92
# Direction: Increase the global multiplier for the `port` class from 1.0 to 1.08 to address the coastal underpre
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    p = {"prior_w": 2.0, "emp_max": 2.0, "smooth_alpha": 0.1}
    ...
    smooth_pred = uniform_filter(pred, size=(3, 3, 1))
    pred = pred * (1.0 - p["smooth_alpha"]) + smooth_pred * p["smooth_alpha"]
    ...
    probs = np.maximum(pred, 0.0)
    
    mask_nonzero = probs > 0
    probs[mask_nonzero] = np.maximum(probs[mask_nonzero], 0.005)

    probs[~mountain_mask, 5] = 0.0
    probs[~coastal_mask, 2] = 0.0

    row_sums = probs.sum(axis=-1, keepdims=True)
    probs = np.divide(probs, row_sums, out=np.ones_like(probs)/NUM_CLASSES, where=row_sums>0)
