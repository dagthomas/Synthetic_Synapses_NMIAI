# Score: avg=83.49
# Direction: Increase the `port` class `coarse_weight` calibration parameter from 1.0 to 1.15 to address coastal 
# Error: None
...
def experimental_pred_fn(state: dict, global_mult,
    ...
    for r in range(MAP_H):
        for c in range(MAP_W):
            terrain = grid[r, c]
            terr_class = terrain_to_class(terrain)
            
            for k in range(NUM_CLASSES):
                probs[r, c, k] = pred[r, c, k]
            ...
            for k in range(NUM_CLASSES):
                if probs[r, c, k] > 0:
                    probs[r, c, k] = max(probs[r, c, k], 0.005)
                    
            row_sum = probs[r, c].sum()
            if row_sum > 0:
                probs[r, c] /= row_sum
            else:
                probs[r, c, terr_class] = 1.0
