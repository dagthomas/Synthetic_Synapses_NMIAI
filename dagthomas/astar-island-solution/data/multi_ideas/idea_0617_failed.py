...
# Direction: Increase the `settlement` class `clamp_m` upper bound from 2.5 to 3.0 to allow for higher peak proba
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # 5. Global Multipliers with modified settlement clamp
    clamp_m = np.array([2.0, 2.0, 2.0, 3.0, 2.0, 2.0])
    mults = np.clip(ratio, 1.0 / clamp_m, clamp_m)
    pred *= mults

    # 6. Terrain masking and floor
    coastal_mask = _build_coastal_mask(grid)
    for r in range(MAP_H):
        for c in range(MAP_W):
            terrain = grid[r, c]
            allowed = terrain_to_class[terrain]
            
            mask = np.zeros(NUM_CLASSES)
            mask[allowed] = 1.0
            if allowed == 1 and coastal_mask[r, c]:
                mask[2] = 1.0
            ...
            if s > 1e-6:
                ...
                pred[r, c] = mask / max(mask.sum(), 1.0)
                
            nz = pred[r, c] > 0
            pred[r, c, nz] = np.maximum(pred[r, c, nz],
    return probs