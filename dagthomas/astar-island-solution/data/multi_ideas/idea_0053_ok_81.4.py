# Score: avg=81.36
# Direction: Adjust the `settlement` class clamp range from [0.15, 2.5] to [0.12, 2.65] to reduce tail-end trunca
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # Adjust settlement class clamp range from [0.15, 2.5] to [0.12, 2.65]
    pred[:, :, 1] = np.clip(pred[:, :, 1], 0.12, 2.65)
    ...
    # Port (class 2) = 0 on non-coastal cells
    pred[~coastal_mask, 2] = 0.0

    # Set floor for nonzero classes
    pred = np.maximum(pred, 0.005)

    # Re-apply mountain/port constraints after floor
    pred[~is_mountain, 5] = 0.0
    # Port (class 2) = 0 on non-coastal cells
    pred[~coastal_mask, 2] = 0.0
