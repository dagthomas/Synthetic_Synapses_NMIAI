...
# Direction: Adjust the `settlement` class clamp range from `[0.15, 2.5]` to `[0.12, 2.75]` to allow the Bayesian
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # Settlement clamp modification to [0.12, 2.75]
    pred[:, :, 1] = np.clip(pred[:, :, 1], 0.12, 2.75)

    # Sim prediction blending
    if sim_pred is not None:
        pred = pred * (1.0 - sim_alpha) + sim_pred * sim_alpha

    # Obs expansion radius suppression
    if obs_expansion_radius is not None and len(settlements) > 0:
        ...
        pred[dist > obs_expansion_radius, 1] = 0.005
