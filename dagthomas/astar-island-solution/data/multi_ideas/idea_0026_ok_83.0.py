# Score: avg=82.99
# Direction: Increase the global multiplier for the `port` class from 1.0 to 1.10 to mitigate coastal underpredic
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # Increase the global multiplier for the port class from 1.0 to 1.10
    ratio[2] *= 1.10
    ...
    # Re-apply masks after floor to strictly enforce 0 on invalid cells
    pred[:, :, 2] = np.where(coastal, pred[:, :, 2], 0.0)
    pred[:, :, 5] = np.where(grid == 5, pred[:, :, 5], 0.0)
