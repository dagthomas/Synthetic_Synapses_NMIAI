...
# Direction: Adjust the `settlement` class `clamp_range` from `[0.15, 2.5]` to `[0.12, 2.65]` to allow the empiri
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    if '_load_params' in globals():
        ...
        p = {"prior_w": 1.0, "emp_max": 2.0, "growth_front_mult": 0.5}
    ...
    # Multipliers with adjusted settlement clamp_range
    pred[:, :, 0] *= np.clip(ratio[0], 0.5, 1.5)
    pred[:, :, 1] *= np.clip(ratio[1], 0.12, 2.65)
    pred[:, :, 2] *= np.clip(ratio[2], 0.2, 2.0)
    pred[:, :, 3] *= np.clip(ratio[3], 0.2, 2.0)
    pred[:, :, 4] *= np.clip(ratio[4], 0.2, 2.0)
    pred[:, :, 5] *= np.clip(ratio[5], 0.5, 1.5)

    if obs_expansion_radius is not None and len(settlements) > 0:
        sett_coords = np.array(settlements)
        r_coords, c_coords = np.indices((MAP_H, MAP_W))
        dist = np.full((MAP_H, MAP_W), 10000)
        for sr, sc in sett_coords:
            d = np.abs(r_coords - sr) + np.abs(c_coords - sc)
