# Score: avg=73.90
# Direction: Adjust the `settlement` class clamp range from [0.15, 2.5] to [0.2, 2.0] to reduce high-entropy outl
# Error: None
...
def experimental_pred_fn(state: dict, global_mult,
    ...
    if obs_expansion_radius is not None and len(settlements) > 0:
        sett_mask = np.zeros(grid.shape, dtype=bool)
        for sr, sc in settlements:
            if 0 <= sr < grid.shape[0] and 0 <= sc < grid.shape[1]:
                sett_mask[sr, sc] = True
        dist = distance_transform_cdt(~sett_mask, metric='taxicab')
        pred[dist > obs_expansion_radius, 4] = 0.005
