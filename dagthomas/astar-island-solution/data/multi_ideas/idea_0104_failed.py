# FAILED: Compilation failed: '(' was never closed (<string>, line 96)
# Direction: Increase the settlement class lower clamp from 0.0 to 0.05 to prevent extreme entropy-weighted KL pe
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    if is_sett.any():
        ...
        dist_map = np.full((MAP_H, MAP_W), 100.0)

    pred = pred * mult

    if obs_expansion_radius is not None and is_sett.any():
        out_of_bounds = dist_map > obs_expansion_radius
        pred[out_of_bounds, 1] = 0.0
        pred[out_of_bounds, 2] = 0.0

    # Settlement class lower clamp modification
    pred[:, :, 1] = np.maximum(pred[:, :, 1], 0.05)
    ...
    probs = pred / np.maximum(pred.sum(axis=-1, keepdims=True), 1
    return probs