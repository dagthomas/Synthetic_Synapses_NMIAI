...
# Direction: Increase the settlement class `clamp_m` upper bound from 2.5 to 2.7 to allow empirical features more
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # 5. Ratio Multipliers
    clamp_m = np.clip(ratio, 0.1, 2.0)
    clamp_m[1] = np.clip(ratio[1], 0.1, 2.7)
    pred *= clamp_m
