# Score: avg=44.53
# Direction: Increase the `settlement` class `smoothing_kernel` size from 3 to 5 to better calibrate the probabil
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    if obs_expansion_radius is not None:
        init_settlements = np.zeros((MAP_H, MAP_W), dtype=bool)
        for (r, c_idx) in settlements:
            init_settlements[r, c_idx] = True
        if init_settlements.any():
            dist = distance_transform_cdt(~init_settlements, metric='chessboard')
            pred[dist > obs_expansion_radius, 3] = 0.0

    smoothed = np.zeros_like(pred)
    for c in range(NUM_CLASSES):
        k_size = 5 if c == 3 else 3
        smoothed[:, :, c] = uniform_filter(pred[:, :, c], size=k_size)
    pred = smoothed

    coastal_mask = _build_coastal_mask(grid)
    for i in range(MAP_H):
        for j in range(MAP_W):
            tc = terrain_to_class(grid[i, j])
            ...
            for c in range(NUM_CLASSES):
                if pred[i, j, c] > 0:
                    pred[i, j, c] = max(pred[i, j, c], 0.005)
