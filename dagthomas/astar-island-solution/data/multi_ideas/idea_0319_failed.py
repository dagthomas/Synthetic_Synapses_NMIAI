...
# Direction: Adjust the `settlement` class `clamp_range` from [0.15, 2.5] to [0.12, 2.8] to better capture the pe
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    for c in range(NUM_CLASSES):
        pred[:, :, c] = np.clip(pred[:, :, c], clamp_ranges[c][0], clamp_ranges[c][1])

    for c in range(NUM_CLASSES):
        pred[:, :, c] = uniform_filter(pred[:, :, c], size=p["smooth_size"])
    ...
    if obs_overlay is not None:
        obs_mask, obs_class = obs_overlay
        pred[obs_mask] = 0.0
        pred[obs_mask, obs_class] = 1.0

    if sett_survival is not None:
        surv_mask, surv_prob = sett_survival
        pred[surv_mask, 1] = surv_prob

    if sim_pred is not None:
        pred = pred * (1.0 - sim_alpha) + sim_pred * sim_alpha

    if growth_front_map is not None:
        pred[:, :, 1] *= np.maximum(1.0, growth_front_map * 2.0)
