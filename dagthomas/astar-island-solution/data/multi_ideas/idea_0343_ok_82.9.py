# Score: avg=82.92
# Direction: Increase the `coarse_max` calibration weight from 2.0 to 2.3 to better anchor regional settlement pr
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    for r in range(MAP_H):
        for c in range(MAP_W):
            ...
            mask = probs[r, c] > 0
            if np.any(mask):
                probs[r, c, mask] = np.maximum(probs[r, c, mask], 0.005)
