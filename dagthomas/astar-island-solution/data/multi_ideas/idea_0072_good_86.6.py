# Score: avg=86.57
# Direction: Increase the `coarse_max` calibration weight from 2.0 to 3.8 to better leverage higher-level buckete
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    cal_params = {
        ...
        'cal_coarse_base': 0.5, 'cal_coarse_divisor': 100.0, 'cal_coarse_max': 3.8,
    ...
    if obs_expansion_radius is not None:
        if is_sett.any():
            ...
            pred[suppress_mask, 1] = 0.0
            pred[suppress_mask, 2] = 0.0
        else:
            pred[:, :, 1] = 0.0
            pred[:, :, 2] = 0.0
    ...
    # Floor
    pred = np.where(pred > 0, np.maximum(pred, 0.005), pred)
