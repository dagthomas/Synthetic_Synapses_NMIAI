# Score: avg=86.70
# Direction: Increase the settlement class clamp upper bound from 2.5 to 2.75 to allow higher peak probabilities 
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    mult[1] = np.clip(mult[1], 0.15, 2.75)
    ...
    if is_sett.any():
        ...
        probs[:, :, 1] *= np.where(dist_map <= 2, 1.2, 1.0)
        probs[:, :, 2] *= np.where(dist_map <= 2, 1.2, 1.0)
        if obs_expansion_radius is not None:
            probs[:, :, 1] = np.where(dist_map > obs_expansion_radius, 0.0, probs[:, :, 1])
            probs[:, :, 2] = np.where(dist_map > obs_expansion_radius, 0.0, probs[:, :, 2])

    coastal = _build_coastal_mask(grid)
    probs[:, :, 2] = np.where(coastal, probs[:, :, 2], 0.0)
    probs[:, :, 5] = np.where(grid == 5, probs[:, :, 5], 0.0)
