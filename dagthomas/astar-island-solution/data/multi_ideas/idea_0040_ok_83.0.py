# Score: avg=83.03
# Direction: Increase the `settlement` class calibration upper clamp limit from 2.5 to 3.0 to allow for higher pe
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    if obs_expansion_radius is not None and obs_expansion_radius > 0:
        init_sett_mask = (grid == 1)
        if init_sett_mask.any():
            dist = distance_transform_cdt(~init_sett_mask)
