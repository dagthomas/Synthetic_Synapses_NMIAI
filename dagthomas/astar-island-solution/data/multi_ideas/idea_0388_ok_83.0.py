# Score: avg=83.00
# Direction: Adjust the `settlement` class `clamp_range` from `[0.15, 2.5]` to `[0.12, 2.8]` to better capture se
# Error: None
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    for c in range(NUM_CLASSES):
        if c == 3:
            c_ratio = np.clip(ratio[c], 0.12, 2.8)
        else:
            c_ratio = np.clip(ratio[c], 0.15, 2.5)
        pred[:, :, c] *= c_ratio
