...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers,
    ...
    # Increase the global multiplier for the port class (index 2) to 1.12
    ratio[2] *= 1.12
    ...
    # Blend with simulation predictions if available
    if sim_pred is not None:
        probs = (1 - sim_alpha) * probs + sim_alpha * sim_pred
    ...
    for c in range(5):
        probs[:, :, c] = np.where(mountain_mask, 0.0, probs[:, :, c])
