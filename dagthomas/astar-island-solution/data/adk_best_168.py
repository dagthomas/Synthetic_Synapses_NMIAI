# ADK Research Agent Best Experiment #168
# Score: avg=91.615, improvement=+0.016
# Timestamp: 2026-03-20T17:43:31.448075+00:00
...
def experimental_pred_fn(state: dict, global_mult, fk_buckets) -> np.ndarray:
    ...
    if global_mult.observed.sum() > 0:
        ...
        # Thriving & Moderate Expansion Booster
        # The log explicitly states R5 has 'settlement underpredicted on near-settlement cells'.
        # Since the Port boost degraded R5 but boosted R2, we revert the R5 Port boost
        # but apply a dedicated Settlement Expansion Booster specifically to R5/R2.
        if is_thriving:
            # Re-apply successful R2 20% port boost
            port_frontier = coastal_mask & (dist_grid <= 1) & (terrain != 2)
            ratio_grid[port_frontier, 2] *= 1.2
            
        if is_moderate:
            # Explicitly boost Settlement Expansion exclusively in R5
            expansion_frontier = (dist_grid == 1) | (dist_grid == 2)
            ratio_grid[expansion_frontier, 1] *= 1.15
