# ADK Research Agent Best Experiment #190
# Score: avg=91.749, improvement=+0.019
# Timestamp: 2026-03-20T17:53:55.538691+00:00
...
def experimental_pred_fn(state: dict, global_mult, fk_buckets) -> np.ndarray:
    ...
    # 6. Regime-Adaptive Temperature Softening
    if is_collapse:
        temperature = 0.96   
    else:
        temperature = 0.97  
    ...
    # 7. Forest Multiplier Suppression in Fast Collapse (R3)
    # The Global Multiplier for Forest in R3 naturally scales up heavily (because empty land turns to forest).
    # However, since the baseline GT floor expects massive standard deviation in area distributions, 
    # letting Forest scale completely unrestrained sometimes breaches mathematical bounds resulting in 
    # slightly suboptimal relative Empty probability. 
    # Testing a soft global suppression of 0.95 specifically on Forest specifically in R3 
    # before the final Smart Floor, allowing Empty to marginally recover mass.
    if is_fast_collapse:
        pred[:, :, 4] *= 0.95
        pred /= np.maximum(pred.sum(axis=-1, keepdims=True), 1e-10)

    # 8. Iterative Smart Floor Optimization
