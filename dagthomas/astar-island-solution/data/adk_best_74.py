# ADK Research Agent Best Experiment #74
# Score: avg=91.488, improvement=+0.199
# Timestamp: 2026-03-20T13:14:56.471151+00:00
...
def experimental_pred_fn(state: dict, global_mult, fk_buckets) -> np.ndarray:
    ...
    # Avoid applying temperature to absolute zeros to preserve structural zeros
    # But wait, applying power 1/T to 0 is 0.
    scaled_pred = np.power(dynamic_pred, 1.0 / T)
    
    # We MUST NOT restore structural zeros if they got blurred by T, 
    # but 0^(1/T) is 0, so structural zeros are preserved.
    row_sums = scaled_pred.sum(axis=-1, keepdims=True)
    scaled_pred /= np.maximum(row_sums, 1e-10)
    pred[dynamic_mask] = scaled_pred
