# Experiment: selective_smoothing_and_distance_multipliers
# Hypothesis: Selective Class Smoothing & Distance-Aware Multipliers: Removing Port from spatial smoothing prevents coastal-mass leakage (saving lost probability), while separating global multiplier intensity by distance (higher for current settlements, dampening proxy-expansion) dramatically improves alignment across divergent regimes.
# Timestamp: 2026-03-20T10:49:10.194651+00:00
...
def experimental_pred_fn(state: dict, global_mult, fk_buckets) -> np.ndarray:
    ...
    # In complete collapse (ratio ~ 0.0), trust mid-round observations up to 12.0
    # In thriving periods (ratio ~ 1.5), trust mid-round observations max 6.0 (trust prior more)
    emp_max_weight = np.clip(12.0 - 4.0 * ratio[1], 6.0, 12.0)
    
    N = len(unique_keys)
    lookup = np.zeros((N, 6), dtype=np.float32)
    
    # Distance-Aware Multiplier Power Arrays:
    # Settlement cells track exactly with survival (power=0.75)
    # Expansion cells use survival as a loose proxy, so we dampen the power (power=0.50)
    ...
    for i in range(N):
        ...
        # Apply Multipliers INSIDE the loop based on specific distance bucket
        if dist_bucket == 0:
    ...
    # *Disabled for Port (Class 2)*: Port formation is structurally bound to the coast. 
    # Applying an unmasked uniform filter leaked its probability mass into inland and 
    # ocean tiles where it was structurally zeroed, deleting mass and hurting predictions.
    alpha = 0.75
    for k in [1, 3]:
    ...
    # 7. Floor nonzero classes (0.005 strictly limits absolute lower bounds matching 200 iter sim runs)
