# Experiment: per_class_distance_blend_and_multiplier
# Hypothesis: Per-class distance-dependent blending weight for empirical data. Volatile classes (settlement/port/ruin) near settlements trust empirical data more (weight=2) than stable classes like forest (weight=8). Also applies class-specific powers for global multipliers.
# Timestamp: 2026-03-20T12:31:09.867326+00:00
...
def experimental_pred_fn(state: dict, global_mult, fk_buckets) -> np.ndarray:
    ...
    # Class-specific distance-dependent prior weights
    n_keys = len(unique_keys)
    pw_lookup = np.zeros((n_keys, 6), dtype=float)
    for i, fk in enumerate(unique_keys):
        ...
        if dist >= 3:
            # Far from settlement: trust prior
            w[:] = 10.0
        else:
            # Near settlement: trust empirical for volatile classes
            w[1] = 2.0  # settlement
            w[2] = 2.0  # port
            w[3] = 2.0  # ruin
            w[0] = 3.0  # empty
            w[4] = 8.0  # forest stays stable
    ...
    pw_grid = pw_lookup[idx_grid]  # (H, W, 6)
    ms = params["fk_max_strength"]

    strengths = np.minimum(ms, np.sqrt(cnt_grid))
    strengths_3d = strengths[:, :, np.newaxis]
    
    # Blending with (H,W,6) prior weight
    blended = pred * pw_grid + emp_grid * strengths_3d
    ...
    if global_mult.observed.sum() > 0:
        ...
        # Per-class power: settlement/port/ruin need stronger reaction
