# Experiment: Final Optimized Pipeline
# Hypothesis: Final Pipeline: Adaptive Equilibrium Shift safely targets the EW-KL asymmetry in moderate regimes, Entropy Blending dynamically weights empirical data on uncertain cells, and Coastal Regime Multipliers capture volatile raiding parameters. This combination stably achieves avg ~91.36.
# Timestamp: 2026-03-20T13:05:45.321468+00:00
...
def experimental_pred_fn(state: dict, global_mult, fk_buckets) -> np.ndarray:
    ...
    if global_mult.observed.sum() > 0:
        ...
        # Only boost settlement/port/ruin when the regime is "moderate" (base_ratio near 1.0)
        # In extreme regimes (thriving > 1.5, collapse < 0.5), boosting hurts KL divergence.
