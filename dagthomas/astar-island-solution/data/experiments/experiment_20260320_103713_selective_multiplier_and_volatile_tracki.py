# Experiment: selective_multiplier_and_volatile_tracking
# Hypothesis: 1. Implement selective class scaling: do not apply the global multiplier ratio to Empty and Forest, letting the renormalization naturally fill the probability space without entropy penalties. 
2. Use dynamic tracking strength for volatile classes: Port and Ruin are extremely regime-sensitive and lack the 'mid-simulation illusion' of Settlement, so we use a higher tracking power (0.8 vs 0.6) to trust current-round observations more.
# Timestamp: 2026-03-20T10:37:13.308780+00:00
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers, fk_buckets: FeatureKeyBuckets) -> np.ndarray:
    ...
    for i, fk in enumerate(unique_keys):
        ...
        if total_w > 0:
            ...
            # Equivalent sample size for blending
            fk_counts[i] = max(fine_count, c_count * 0.5, b_count * 0.25)
            
    # Vectorized blending with historical priors
    ...
    # Global Multipliers (Regime Adaptation)
    if global_mult.observed.sum() > 0:
        ...
        # We exclusively scale volatile classes (1: Settlement, 2: Port, 3: Ruin)
        # Empty (0) and Forest (4) are extremely stable and act as probability sinks.
        # Scaling them forcefully causes probability mass leaks that hurt wKL.
        ratio = np.ones(NUM_CLASSES, dtype=float)
        
        # Port and Ruin formation is extremely regime-sensitive (non-linear).
        # We trust the mid-simulation observations more (power=0.8) than settlement (0.6)
        # because Settlement has high "mid-simulation illusion" (alive when observed, dead at end).
        ratio[1] = np.power(raw_ratio[1], 0.6)
        ratio[2] = np.power(raw_ratio[2], 0.8)
        ratio[3] = np.power(raw_ratio[3], 0.8)
