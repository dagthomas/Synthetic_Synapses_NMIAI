# Experiment: empirical_saturation_and_ruin_persistence
# Hypothesis: Saturating empirical blends for stable classes in extreme regimes. In thriving rounds (>1.1) and total collapse (<0.2), variance is low for the dominant Empty/Forest cells. Boosting their blend towards empirical data reduces KL divergence structurally without disrupting the finely-tuned multiplier logic. We also ensure Ruins correctly inherit explicit probability mass when starting as Ruins or when replacing dying settlements in Moderate regimes.
# Timestamp: 2026-03-20T13:05:34.289469+00:00
...
def experimental_pred_fn(state: dict, global_mult, fk_buckets) -> np.ndarray:
    ...
    # REGIME-AWARE EMPIRICAL BLEND SATURATION
    if has_fk.any() and (base_ratio_sett > 1.1 or base_ratio_sett < 0.2):
        c_shifted = np.maximum(0, cnt_grid - 5)
        blend_factor = np.minimum(25, c_shifted) / 25.0 * 0.40
        
        e_emp = emp_grid[:, :, 0]
        f_emp = emp_grid[:, :, 4]
        
        pred[:, :, 0] = pred[:, :, 0] * (1.0 - blend_factor) + e_emp * blend_factor
        pred[:, :, 4] = pred[:, :, 4] * (1.0 - blend_factor) + f_emp * blend_factor
