# Experiment: regime_adaptive_empirical_trust
# Hypothesis: Regime-Adaptive Empirical Trust Scaling: Replaces static weight limit with a linear scaling limit (6.0 - 12.0) based on survival ratio, heavily trusting empirical data in collapse regimes where it's accurate, and suppressing it in thriving regimes where it fails to capture late-game expansion.
# Timestamp: 2026-03-20T10:44:10.025934+00:00
...
def experimental_pred_fn(state: dict, global_mult, fk_buckets) -> np.ndarray:
    ...
    # Scales empirical trust linearly with the regime survival ratio.
    # Total Collapse (ratio=0) -> trust = 12.0 (Death happens early, mid-round empirical is perfectly reliable)
    # Moderate (ratio~1.0) -> trust = 8.0 (Standard blend, mid-round snapshot lacks certainty)
    # Thriving (ratio=1.5+) -> trust = 6.0 (Expansion happens late, empirical misses it so we trust prior)
    ...
    # 7. Floor nonzero classes (0.005 minimum limit enforced by rules)
