# Experiment: distance_aware_dynamic_softening
# Hypothesis: Distance-Aware Dynamic Temperature Softening: Adjusts probability distributions based on regime survival ratio (T=1.0 for collapse, T=1.10 for thriving) exclusively within distance <= 4 of settlements to hedge against spatial uncertainty.
# Timestamp: 2026-03-20T10:29:29.029393+00:00
...
def experimental_pred_fn(state: dict, global_mult, fk_buckets) -> np.ndarray:
    ...
    # Distance-aware Dynamic Temperature Softening
    # Calculates Manhattan distance to the nearest existing settlement
    is_sett = (grid == 1) | (grid == 2)
    dist_map = distance_transform_cdt(~is_sett, metric='taxicab')
    
    # If the simulation is in a collapse regime (ratio -> 0), T -> 1.0 (sharp predictions).
    # If the simulation is surviving/thriving, T -> 1.10 (softer predictions to account for chaos).
    T_max = 1.0 + 0.10 * math.sqrt(min(ratio[1], 1.0))
    T_grid = np.ones((40, 40, 1), dtype=np.float32)
    
    # Only apply the softening in the high-entropy radius (distance <= 4 from settlements)
    T_grid[dist_map <= 4] = T_max
    
    probs = np.power(probs, 1.0/T_grid)
    probs = probs / probs.sum(axis=-1, keepdims=True)
    
    # Spatial smoothing post-processing (alpha=0.75)
    alpha = 0.75
    for k in [1, 2, 3]:
