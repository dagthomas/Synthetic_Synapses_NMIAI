# Experiment: prior_first_multiplier_scaling
# Hypothesis: Apply the Global Multiplier to the `cal_priors` BEFORE blending with the empirical distributions. 

The previous pipeline applied the regime multiplier at the very end. This caused "double-counting" of the regime shift: empirical data naturally reflects the current round's regime, and multiplying it post-hoc forces it further, necessitating dampening (`power=0.6`). By shifting the *historical prior* to match the current regime first, we allow the empirical data to blend cleanly and have the final say, enabling us to safely use a stronger multiplier power (`0.8` to `1.0`) on the priors without overshooting.
# Timestamp: 2026-03-20T10:39:21.712028+00:00
...
def experimental_pred_fn(state: dict, global_mult: GlobalMultipliers, fk_buckets: FeatureKeyBuckets) -> np.ndarray:
    ...
    # =================================================================
    # NEW: Apply Global Multipliers to Priors BEFORE Empirical Blending
    # =================================================================
    if global_mult.observed.sum() > 0:
        ...
        # We use stronger powers because applying to the prior prevents double-counting
        # empirical evidence later.
        ratio[1] = np.power(raw_ratio[1], 0.8)  # Settlement
        ratio[2] = np.power(raw_ratio[2], 1.0)  # Port
        ratio[3] = np.power(raw_ratio[3], 1.0)  # Ruin
        
        for c in (1, 2, 3):
            ratio[c] = np.clip(ratio[c], params.get("mult_sett_lo", 0.15), params.get("mult_sett_hi", 2.5))

        cal_priors *= ratio
        cal_priors /= np.maximum(cal_priors.sum(axis=-1, keepdims=True), 1e-10)
    ...
    # Vectorized blending with adjusted priors
