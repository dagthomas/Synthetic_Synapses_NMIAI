# Experiment: combined_density_port_collapse_dampening
# Hypothesis: Combines class-specific regime dampening, settlement density penalties for dense clusters, proportional collapse mass redistribution, and a port probability boost for coastal cells. This addresses specific error patterns observed in thriving (R2) and moderate (R5) regimes.
# Timestamp: 2026-03-20T12:51:30.966176+00:00
...
def experimental_pred_fn(state: dict, global_mult, fk_buckets) -> np.ndarray:
    ...
    if global_mult.observed.sum() > 0:
        ...
        # Class-specific dampening for volatile vs stable classes
    ...
    # Settlement Density Modifier (only in non-collapse rounds)
    if not collapse_detected:
        ...
        for (sy, sx) in sett_pos:
            if terrain[sy, sx] in (1, 2):
                ...
                # Isolated settlements survive best
                if neighbors == 0:
                    pred[sy, sx, 1] *= 1.15
                    pred[sy, sx, 2] *= 1.15
                elif neighbors == 1:
                    pred[sy, sx, 1] *= 1.05
                    pred[sy, sx, 2] *= 1.05
                else: # Dense clusters collapse due to raiding/competition
                    pred[sy, sx, 1] *= 0.85
                    pred[sy, sx, 2] *= 0.85
    ...
    if collapse_detected:
        ...
        if pos_mask.any():
            ...
            # Proportional redistribution to Empty/Forest
