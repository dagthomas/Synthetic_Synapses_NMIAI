# ADK Research Agent Best Experiment #165
# Score: avg=91.595, improvement=+0.011
# Timestamp: 2026-03-20T17:41:34.727746+00:00
...
def experimental_pred_fn(state: dict, global_mult, fk_buckets) -> np.ndarray:
    ...
    if global_mult.observed.sum() > 0:
        ...
        # Refined Coastal vs Inland Survival Shift for R3
        # In R3 (Fast Collapse), the coastal penalty works beautifully (score jumped from 93.65 to 93.70!)
        # However, it degraded R5 and R4. We will isolate the Coastal vs Inland shift specifically 
        # to the Fast Collapse (R3) regime where raiding heavily accelerates coastal deaths.
        if is_fast_collapse:
            coastal_sett_mask = coastal_mask & (terrain == 1)
            inland_sett_mask = (~coastal_mask) & (terrain == 1)
            
            # Penalize coastal survival heavily
            ratio_grid[coastal_sett_mask, 1] *= 0.8
            # Boost inland survival to maintain global average
