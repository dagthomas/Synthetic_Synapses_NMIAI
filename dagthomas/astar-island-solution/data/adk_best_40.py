# ADK Research Agent Best Experiment #40
# Score: avg=91.237, improvement=+0.012
# Timestamp: 2026-03-20T12:53:13.636751+00:00
...
def experimental_pred_fn(state: dict, global_mult, fk_buckets) -> np.ndarray:
    ...
    # Port Boosting on Coastal Cells (Slightly relaxed in total collapse to avoid mass waste)
    coastal = _build_coastal_mask(terrain)
    port_boost = 1.05 if collapse_detected else 1.25
    pred[coastal, 2] *= port_boost
    
    # NEW STRUCTURAL FEATURE: RUIN RECLAMATION
    # If a settlement dies in R5 (moderate regime), what happens?
    # R5 findings show ruins form strictly around INITIAL SETTLEMENTS.
    # We should ensure that if `collapse_detected` is FALSE but base_ratio_sett is low (~0.5 - 0.8),
    # ruin probability gets specifically tied to the LOST settlement probability.
    if not collapse_detected and base_ratio_sett < 1.0:
        for s in settlements:
            sy, sx = int(s["y"]), int(s["x"])
            # Loss in settlement prob:
            # We don't have exact before/after, but if settlement < 50%, ruin should be somewhat comparable.
            if pred[sy, sx, 1] < 0.5:
                pred[sy, sx, 3] *= 1.25
