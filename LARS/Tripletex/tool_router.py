...
TASK_TOOL_MAP: dict[str, list[str]] = {
    ...
    "create_multiple_departments": ["create_department"],
    ...
    "invoice_with_payment":        ["process_invoice_with_payment"],
    ...
    "project_invoice":             ["process_project_invoice"],
    ...
    "reverse_payment":        ["search_invoices", "get_ledger_postings", "reverse_voucher",
                               "process_reverse_payment"],
...
_PATTERNS: list[tuple[str, list[str], list[str], int]] = [
    ...
    ("delete_employee", ["slett ansatt", "delete employee", "fjern ansatt",
                         ...
                         "fra ansattregisteret",
                         "fjerne fra ansattregisteret", "slette fra ansattregisteret",
                         "fjernes fra ansattregisteret", "slett fra ansattregisteret",
    ...
    ("create_credit_note", ["kreditnota", "kreditere", "credit note", "kreditér",
                            ...
                            "deretter kreditere", "straks kreditere", "så kreditere",
                            "umiddelbart kreditere", "deretter en kreditnota",
                            "opprette en kreditnota", "opprett en kreditnota",
                            "vennligst krediter", "vennligst kreditere",
                            "krediter den umiddelbart", "kreditere den umiddelbart",
                            "ber jeg deg om å kreditere"],
     ["kreditere konto", "debitere konto", "debitere", "bilag", "hovedbok",
      "korreksjonsbilag", "postering", "feilpostering",
      # delete_invoice signals — both pipelines identical, but keep classifier distinct
      "slett faktura", "slett fakturaen", "slette fakturaen",
      "fjerne fakturaen", "fjerne faktura",
      "kanseller faktura", "kansellere faktura", "kansellere fakturaen",
      "annuller faktura", "annullere faktura"], 8),
    ...
    ("delete_invoice", ["slett faktura", "delete invoice", "slett fakturaen",
                        ...
                        "kanseller faktura", "kansellere faktura", "kansellere fakturaen",
                        "kansellert ordre", "kansellert bestilling",
                        "annuller faktura", "annullere faktura", "annullere fakturaen",
                        "umiddelbart kreditere", "krediter den umiddelbart",
                        "kreditere den umiddelbart", "kreditere den med en kreditnota",
                        "krediter den med en kreditnota",
                        "korrigere en feil",
                        "tilbakeføre fakturaen", "reversere fakturaen",
                        "fakturakorrigering", "korrigere faktura", "korrigere fakturaen",
                        "rydde opp", "rydde opp i",
                        "ber jeg deg om å kreditere", "deretter kreditere",
                        "straks kreditere", "så kreditere",
                        "vennligst krediter", "vennligst kreditere",
    ...
    ("order_to_invoice_with_payment", [
        ...
        "opprett en bestilling", "opprette en bestilling",
        "ordren", "i ordren", "til ordren", "legg til i ordren",
        ...
        "pedido", "bestilling", "bestillingen",
        "ordre", "ordren", "ordrelinje",
        "order", "auftrag", "bestellung",
        "commande", "encomenda",
    ], [# Project signals — should go to project_invoice instead
      "prosjekt", "project", "proyecto", "projet", "projekt", "projeto",
      "fastpris", "fastprisprosjekt", "prosjektfaktura", "prosjektleder",
      "project manager", "fixed price project",
      "fakturere prosjekt", "fakturere prosjektet",
      "prosjektfakturering",
      # Credit note signals — should go to create_credit_note instead
      "kreditnota", "kreditere", "kreditér", "credit note",
      "gutschrift", "nota de crédito", "note de crédit",
      "kreditere fakturaen", "kreditera",
      "opprette en kreditnota", "opprett en kreditnota",
      "deretter kreditere", "straks kreditere", "så kreditere"], 10),
    ...
    ("invoice_with_payment", ["faktura betaling", "invoice payment", "registrer betaling",
                              ...
                              "betalt", "betalte", "er betalt", "den er betalt", "ble betalt",
                              "allerede betalt", "already paid", "paid in full",
                              "registrere at den er betalt",
                              "var rask og betalte", "betalte hele",
                              "pagado", "pagada", "pago", "bezahlt", "payé", "payée",
     ...
     # credit note signals must not match invoice_with_payment
     ["pedido", "bestilling",
      "kreditnota", "kreditere", "kreditér", "credit note",
      "gutschrift", "nota de crédito", "note de crédit",
      "kreditere fakturaen", "opprette en kreditnota",
      ...
      "payment returned", "payment bounced", "reverse payment", "revert payment",
      # Salary/payroll signals — should go to salary_with_bonus instead
      "lønnsutbetaling", "lønnskjøring", "lønnstransaksjon", "lønnsslipp",
      "payroll", "salary transaction", "salary slip",
      "gehaltsabrechnung", "lohnabrechnung",
      "traitement salarial", "fiche de paie",
      "processar salário", "folha de pagamento",
      "procesar nómina", "procesar salario"], 9),
    ...
    ("create_multi_line_invoice", ["flere produkter", "multiple products", "flere linjer",
                                   ...
                                   "fleire produkt",
                                   # Norwegian multi-item synonyms
                                   "tre ulike", "to ulike", "fire ulike",
                                   "tre forskjellige", "to forskjellige",
                                   "tre ting", "to ting", "fire ting",
                                   "tre poster", "to poster",
                                   "tre linjer", "to linjer", "fire linjer",
                                   "faktureres for tre", "faktureres for to", "faktureres for fire",
                                   # Digit-based count patterns (LLM often uses digits not words)
                                   "3 produkter", "2 produkter", "4 produkter",
                                   "3 tjenester", "2 tjenester", "4 tjenester",
                                   "3 varer", "2 varer", "4 varer",
                                   "3 linjer", "2 linjer", "4 linjer",
                                   "3 poster", "2 poster", "4 poster",
                                   "3 ulike", "2 ulike", "4 ulike",
                                   "3 forskjellige", "2 forskjellige", "4 forskjellige",
                                   "med 3", "med 2", "med 4",
                                   "inneholde:", "inneholder:",
                                   "disse produktene", "disse tjenestene", "disse varene",
                                   # Definite form (produktene vs produkter)
                                   "tre produktene", "to produktene", "fire produktene",
                                   "3 produktene", "2 produktene", "4 produktene",
                                   "disse tre", "disse to", "disse fire",
                                   "inneholde disse", "inneholder disse",
                                   ...
                                   "three lines", "three products", "three items",
                                   "two lines", "two products", "two items",
                                   "zwei produkte", "deux produits",
                                   "dos productos", "duas linhas", "dos líneas",
                                   "deux lignes", "zwei positionen"],
     # Negative: order-first workflow signals → order_to_invoice_with_payment instead
     # Project creation signals → project_invoice / create_project instead
     # NOTE: do NOT use bare "prosjekt" — product names like "Prosjektledelse" contain it
     ["bestilling", "pedido", "order to invoice", "bestillingen",
      "konverter bestilling", "convert order",
      "nytt prosjekt", "opprette prosjektet", "opprett prosjektet",
      "new project", "create project"], 9),
    ...
    ("register_expense_receipt", [
        ...
        # Single-word fallbacks for English prompts
        "receipt registered", "receipt posted", "this receipt",
        "receipt", "debit account", "debitkonto",
        "vat treatment", "mva-behandling", "correct vat", "riktig mva",
    ...
    ("create_supplier_invoice", ["leverandørfaktura", "supplier invoice", "inngående faktura",
                          ...
                          "leverandørfaktura",
                          # Flexible Norwegian supplier invoice patterns
                          "faktura fra en leverandør", "faktura fra leverandør",
                          "faktura fra leverandøren",
                          "mottatt en faktura", "mottatt faktura",
                          "mottatt en ny faktura",
                          "inngående mva", "riktig inngående mva",
                          "registrerer fakturaen",
                          "opprette leverandøren",
     ...
     ["depreciation", "avskrivning", "month-end", "trial balance",
      ...
      "årsoppgjør", "årsavslutning", "periodisering",
      ], 8),
    ...
    ("create_employee_with_employment", ["ansettelsesforhold", "employment", "arbeidsforhold",
                                         ...
                                         # Multi-word create+employment patterns
                                         "opprette ansattkortet og sette opp",
                                         "ansattkortet og arbeidsforhold",
                                         "ansattkortet og sette opp et",
                                         "ny medarbeider",
                                         "nyansatt med arbeidsforhold",
                                         "nyansettelse",
                                         "første arbeidsdag", "forste arbeidsdag",
     ...
     ["prosjekt", "project", "proyecto", "projeto", "projet", "projekt",
      # Salary/payroll discriminators — should go to salary_with_bonus
      "lønnskjøring", "payroll", "lønnstransaksjon", "salary transaction",
      "lønnsslipp", "salary slip", "bonus", "eingongsbonus",
      "grunnløn", "grunnlønn", "grundgehalt",
      "gehaltsabrechnung", "lohnabrechnung",
      "folha de pagamento", "nómina", "fiche de paie", "bulletin de paie",
      "processar salário", "procesar salario", "traitement salarial"], 7),
    ...
    ("create_project_with_billing", [
     ...
     ["artikkel", "artikkelnummer", "varenummer", "produktnummer",
      "opprett produkt", "create product", "nytt produkt", "ny vare", "ny artikkel",
      "ekskl mva", "eksklusiv mva", "priceexcludingvat",
      # Multi-line invoice discriminators — no project involved
      "flere produkter", "flere varer", "flere linjer", "multiple products",
      "multiple items", "multiple lines", "flerlinjet", "multi-line",
      "três produtos", "three products", "drei produkte", "trois produits",
      "tres productos", "varios productos", "plusieurs produits",
      "mehrere produkte", "diversos produtos"], 13),
    ...
    ("project_lifecycle", ["ciclo de vida", "project lifecycle", "prosjektlivssyklus",
     ...
     ["artikkel", "artikkelnummer", "varenummer", "produktnummer",
      "opprett produkt", "create product", "nytt produkt", "ny vare", "ny artikkel",
      "ekskl mva", "eksklusiv mva", "priceexcludingvat"], 12),
    ...
    ("project_invoice", ["prosjekt faktura", "project invoice", "proyecto factura",
                         ...
                         "prix fixe", "festpreis",
                         "fastprisprosjekt", "fixed price project",
                         ...
                         "facturer le projet",
                         "fakturere prosjektet", "prosjektfaktura",
                         "faktura for prosjekt", "faktura til prosjekt",
                         "fakturere kunden for prosjekt",
                         "fakturering", "prosjektfakturering",
                         # Imperative forms (accent stripped: é→e)
                         "fakturer prosjektet", "fakturer prosjekt",
                         # Project + invoice in same sentence (common LLM patterns)
                         "faktura for prosjektet", "faktura til prosjektet",
                         "en faktura for prosjektet", "lag faktura for prosjektet",
                         "opprett faktura for prosjektet",
                         "prosjektet faktureres", "prosjektet skal faktureres",
                         "invoice for the project", "invoice the project",
                         "factura para el proyecto", "factura para o projeto",
                         "facture pour le projet", "rechnung fur das projekt",
                         # Hourly project invoice (register hours → invoice)
                         "horas", "hours", "timer", "heures", "stunden",
                         "tarifa por hora", "hourly rate", "timepris", "taux horaire",
                         "stundensatz", "taxa horaria",
                         "horas registradas", "hours logged", "timer registrert",
                         "basada en las horas", "based on hours", "basert pa timer",
                         "basee sur les heures", "auf stundenbasis",
                         "registre horas", "register hours", "registrer timer",
                         "registrar horas", "enregistrer les heures", "stunden erfassen",
                         "registre timer"],
    ...
    ("create_invoice", ["opprett faktura", "opprette faktura", "ny faktura",
                        ...
                        "opprett en faktura", "lage en faktura",
                        "opprette en faktura", "sende en faktura",
                        "fakturere", "fakturer",
                        "create invoice", "new invoice", "create an invoice",
                        ...
                        "fakturaen", "fakturaer",
                        "regning", "fakturadato", "forfallsdato",
     ...
     ["slett", "delete", "kreditnota", "kreditere", "credit note",
      ...
      "payment returned", "payment bounced", "reverse payment", "revert payment",
      # Past-tense payment signals (should go to invoice_with_payment)
      "betalt", "betalte", "er betalt", "ble betalt", "allerede betalt",
      "already paid", "paid in full", "pagado", "bezahlt", "payé",
      # Multi-line signals — should go to create_multi_line_invoice instead
      "flere produkter", "multiple products", "flere linjer", "multiple lines",
      "tre produkter", "to produkter", "fire produkter",
      "følgende produkter", "følgende varer",
      "tre ulike", "to ulike", "fire ulike",
      "tre forskjellige", "to forskjellige",
      "tre ting", "to ting", "fire ting",
      "three products", "two products", "three lines", "two lines",
      "three items", "two items",
      "tre linjer", "to linjer", "fire linjer",
      "f\u00f8lgende", "following items", "following products",
      "varios productos", "vários produtos", "plusieurs produits", "mehrere produkte",
      # Digit-based multi-line signals
      "3 produkter", "2 produkter", "4 produkter",
      "3 tjenester", "2 tjenester", "4 tjenester",
      "3 varer", "2 varer", "4 varer",
      "3 linjer", "2 linjer", "4 linjer",
      ...
      "inneholde disse", "inneholder disse",
      # Project signals — should go to project_invoice instead
      "prosjektleder", "fastpris", "fastprisprosjekt",
      "project manager", "fixed price project",
      "opprette prosjektet", "opprett prosjektet",
      "fakturere prosjekt", "fakturere prosjektet",
      "prosjektfaktura", "prosjektfakturering"], 4),
    ...
    ("create_contact", ["kontaktperson", "contact person",
     ...
     ["oppdater kontakt", "oppdatere kontakt", "endre kontakt",
      ...
      # Customer-specific fields — contact persons don't have these
      "organisasjonsnummer", "org.nr", "orgnr", "org nummer",
      "postnummer", "postkode", "poststed",
      "organization number", "postal code",
      "organisationsnummer", "postleitzahl",
      "leverandor", "supplier", "faktura", "invoice"], 6),
    ...
    ("update_employee", ["oppdater ansatt", "update employee", "endre ansatt",
     ...
     ["slett", "delete", "eliminar", "excluir", "supprimer", "löschen",
      # Create+employment context — should go to create_employee_with_employment
      "arbeidsforhold", "ansettelsesforhold", "ansettelse",
      "ny medarbeider", "nyansatt",
      "første arbeidsdag", "forste arbeidsdag",
      "tiltredelse", "startdato", "stillingsprosent"], 8),
    ...
    ("update_department", ["oppdater avdeling", "update department", "endre avdeling", "oppdatere avdeling",
                           ...
                           "avdelingen skal hete",
                           "rename department", "umbenennen"],
     ["slett", "delete", "eliminar", "excluir", "supprimer", "löschen",
      "produkt", "product", "producto", "produit", "produto", "vare",
      "prosjekt", "project", "faktura", "invoice",
      "opprett", "opprette", "ny avdeling", "new department",
      "neue abteilung", "crear departamento", "novo departamento",
      "créer département", "nouveau département"], 8),
    ...
    ("create_dimension", ["dimensjon", "dimensjonen", "dimensjoner",
                          "dimension", "dimensions",
                          "buchhaltungsdimension", "regnskapsdimensjon", "accounting dimension",
                          ...
                          "ny dimensjon", "opprett dimensjon", "opprette dimensjon",
                          # French
    ...
    ("correct_ledger_errors", [
    ...
    ], [# Payment discriminators — should go to reverse_payment
      "tilbakeføre betaling", "reversere betaling", "reverser betaling",
      "reverse payment", "revert payment", "payment returned", "payment bounced",
      "betaling returnert", "betaling avvist", "betaling retur",
      "betaling ble returnert", "returnert av banken",
      "returned by the bank", "bounced by the bank",
      "reverter pagamento", "pagamento devolvido", "estornar pagamento",
      "revertir pago", "pago devuelto", "anular pago",
      "contrepasser paiement", "paiement retourné",
      "zahlung zurückbuchen", "zahlung stornieren", "rücklastschrift"], 15),
    ...
    ("create_ledger_voucher", ["bilag", "voucher", "korrigeringsbilag", "korreksjonsbilag",
     ...
     ["tilbakeføre", "reversere", "reverse", "slett", "delete",
      ...
      "gastos prepagados", "forskuddsbetalt", "prepaid expenses",
      # Travel expense discriminators — prevent travel prompts from matching ledger voucher
      "reiseregning", "travel expense", "reiseutlegg", "diett", "per diem",
      "kjøregodtgjørelse", "mileage", "overnatting", "hotell",
      "note de frais", "reisekosten", "despesas de viagem", "gastos de viaje",
      "viáticos", "tagegeld", "spesen"], 7),
    ...
    ("create_employee", ["nyansettelse", "ny ansettelse", "nyansatt", "nytilsett",
                         ...
                         "neueinstellung"],
     ["slett", "delete", "eliminar", "excluir", "supprimer", "löschen",
      # Employment-context discriminators — should go to create_employee_with_employment
      "ansettelsesforhold", "arbeidsforhold", "employment details",
      "startdato", "start dato", "start date", "tiltredelse", "tiltredelsesdato",
      "stillingsprosent", "stillingstittel", "yrkeskode", "occupation code",
      "første arbeidsdag", "forste arbeidsdag",
      "contrato de trabajo", "contrato laboral", "contrat de travail",
      "arbeitsvertrag", "arbeitsverhältnis",
      "contrato de trabalho", "relação de emprego",
      "fecha de inicio", "date d'embauche", "eintrittsdatum",
      "data de início", "data de inicio"], 10),
    ...
    # ── Create multiple departments (before single department) ──
    ("create_multiple_departments", [
        "tre avdelinger", "three departments", "drei abteilungen", "três departamentos",
        "tres departamentos", "trois départements", "trois departements",
        "tre avdelingar",  # nynorsk
        "flere avdelinger", "multiple departments", "mehrere abteilungen",
        "vários departamentos", "varios departamentos", "plusieurs départements",
        "plusieurs departements",
        "to avdelinger", "two departments", "zwei abteilungen",
        "fire avdelinger", "four departments", "vier abteilungen",
        "fem avdelinger", "five departments", "fünf abteilungen", "funf abteilungen",
    ], ["slett", "delete", "eliminar", "excluir", "supprimer", "löschen",
        "oppdater", "update", "endre"], 15),
    ...
    ("create_customer", ["kunde", "customer", "cliente", "client", "klient",
                         "ny kunde", "new customer", "nuevo cliente", "nouveau client",
                         "opprett kunde", "registrer kunde", "legg inn kunde",
                         "registrere kunde", "opprette kunde",
                         "legge inn kunde", "legge dem inn",
                         "ny klient", "new client", "novo cliente",
                         "neuer kunde", "nueva cuenta",
                         # Org number = always a company/customer, not a contact
                         "organisasjonsnummer", "org.nr", "orgnr",
     ...
     ["slett", "delete", "oppdater", "update", "endre",
      ...
      "actualizar", "atualizar", "modifier", "ändern", "aktualisieren",
      "mettre a jour", "mettre à jour",
      "opprett faktura", "lag faktura", "send faktura",
      "opprett en faktura", "lage en faktura",
      "create invoice", "new invoice", "create an invoice",
      # Date/payment keywords — strong invoice signals (not in customer prompts)
      "fakturadato", "forfallsdato", "invoice date", "due date",
      "kreditnota", "credit note",
      "betaling", "payment", "paiement", "pago", "zahlung"], 4),
    ("create_product", ["produkt", "product", "producto", "produit", "produto", "vare",
                        "artikkel", "artikler", "artikkelnummer", "varenummer", "produktnummer",
                        "nytt produkt", "ny vare", "ny artikkel",
                        "opprett produkt", "opprette produkt", "opprett en vare",
                        "create product", "new product", "crear producto", "novo produto",
                        "pris ekskl", "pris eksklusive", "pris eksklusiv",
                        "ekskl mva", "eksklusiv mva", "excluding vat",
                        "priceexcludingvat", "price excluding"],
