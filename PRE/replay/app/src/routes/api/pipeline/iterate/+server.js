	...
	const stream = new ReadableStream({
		start(controller) {
			...
			(async () => {
				...
				// Orders and solutions are stored in PostgreSQL (date-keyed).
				// Clean up any stale flat files from previous days to avoid confusion.
				for (const diff2 of ["easy", "medium", "hard", "expert", "nightmare"]) {
					const dir = resolve(solDir, diff2);
					if (existsSync(dir)) {
						for (const file of ['best.json', 'capture.json', 'meta.json', 'dp_plan.json', 'route_table.json']) {
							const p = resolve(dir, file);
							try { if (existsSync(p)) rmSync(p); } catch (e) {}
						}
					}
				}
