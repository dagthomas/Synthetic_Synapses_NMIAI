#!/usr/bin/env python3
"""Nightmare ILS using LMAPF solver (best single-run at 302) for suffix reconstruction.

Key insight: LMAPF scores 302 vs V6's 254. Using it as the ILS suffix should
push the ceiling higher than the 467 achieved with V6-based ILS.

Usage: python nightmare_lmapf_ils.py --seed 7005 --max-time 600
"""
...
def run_lmapf_suffix(state, all_orders, from_round, num_rounds):
    """Run LMAPF solver from given state."""
...
def run_v6_suffix(state, all_orders, from_round, num_rounds, ddw=0.4, oa=3):
        ...
        def _assign_item(self, bot_pos, needed, assigned_counts, claimed,
            ...
            for tid, need_count in needed.items():
                ...
                if strict:
                    ...
                    max_assign = need_count + self._oa_bonus
                else:
                    max_assign = need_count + 1
                if assigned_counts.get(tid, 0) >= max_assign: continue
                bonus = type_bonus.get(tid, 0) if type_bonus else 0
                for item_idx, adj_cells, item_zone in self.type_items.get(tid, []):
                    if item_idx in claimed: continue
                    if zone_filter >= 0 and item_zone != zone_filter: continue
    ...
    solver.allocator = OAAlloc(ms, tables, solver.drop_zones,
                               max_preview_pickers=99, drop_d_weight=ddw,
                               over_assign_bonus=oa)
    action_log = []
    for rnd in range(from_round, num_rounds):
        state.round = rnd
        actions = solver.action(state, all_orders, rnd)
...
def best_suffix(state_template, all_orders, from_round, num_rounds):
    """Try LMAPF + multiple V6 params, return best."""
    best_score = -1
    best_actions = None
    best_tag = None

    # LMAPF (best single-run solver)
    s = copy.deepcopy(state_template)
    score, actions = run_lmapf_suffix(s, all_orders, from_round, num_rounds)
    if score > best_score:
        best_score = score
        best_actions = actions
        best_tag = 'lmapf'

    # V6 with different params
    v6_params = [(0.3, 3), (0.4, 2), (0.4, 3), (0.5, 3), (0.6, 3)]
    for ddw, oa in v6_params:
        s = copy.deepcopy(state_template)
        score, actions = run_v6_suffix(s, all_orders, from_round, num_rounds,
                                       ddw=ddw, oa=oa)
        if score > best_score:
            best_score = score
            best_actions = actions
            best_tag = f'v6({ddw},{oa})'

    return best_score, best_actions, best_tag


def run_ils(seed, max_time=600, initial_actions=None, verbose=True):
    t_start = time.time()
    ...
    # Fresh multistart with best_suffix
    s = copy.deepcopy(state0)
    ms_score, ms_actions, ms_tag = best_suffix(s, all_orders, 0, num_rounds)
    if ms_score > best_score:
        best_score = ms_score
        best_actions = ms_actions
    print(f'Multistart best: {ms_score} [{ms_tag}] ({time.time()-t_start:.1f}s)',
          file=sys.stderr)

    global_best = best_score
    global_actions = [list(a) for a in best_actions]

    # Phase 2: ILS with multi-solver suffix
    cp_interval = 10
    ...
    while True:
        ...
        # Try multi-solver suffix (LMAPF + V6 variants)
        suffix_score, suffix_actions, suffix_tag = best_suffix(
        ...
        if suffix_score > global_best:
            ...
            if verbose:
                print(f'  ILS {ils_iter} [{elapsed:.0f}s]: BEST {global_best} '
                      f'(kick@{kick_start}, {suffix_tag})', file=sys.stderr)
        else:
            no_improve += 1

        # Trajectory search phase
        traj_budget = min(remaining * 0.15, 20)
        traj_t = time.time()
        while time.time() - traj_t < traj_budget:
            ...
            # LMAPF suffix for traj search
            traj_score, traj_log = run_lmapf_suffix(
                ts, cp_o, tr + 1, num_rounds)
        ...
        if no_improve >= 5:
            s = copy.deepcopy(state0)
            rs, ra, rt = best_suffix(s, all_orders, 0, num_rounds)
            if rs > 200:
                best_score = rs
                best_actions = ra
                checkpoints = rebuild_checkpoints(seed, best_actions, num_rounds, cp_interval)
                no_improve = 0
                if verbose:
                    print(f'  RESTART [{time.time()-t_start:.0f}s]: {rs} [{rt}]',
                          file=sys.stderr)

        if verbose and ils_iter % 2 == 0:
